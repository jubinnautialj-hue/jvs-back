    ...
    <div class="cont">
      ...
      <el-form
        ...
        <el-tabs v-model="tabkey" type="card" @tab-click="handleClick">
          <el-tab-pane v-for="(ti, tix) in data.dicData" :key="'tab'+ti.name" :label="ti.label" :name="ti.name">
            <span slot="label"><span>{{ti.label}}</span><i v-if="data.detachData" class="el-icon-setting" style="margin-left: 5px;" @click.stop="tabItemSetting(ti, tix)"></i></span>
            <template>
                  ...
                  >
                    ...
                    <div :class="['box', 'step', 'formbox', 'tableForm', 'flowTable', 'flowNode', 'reportTable'].indexOf(ci.type) === -1 ? 'headeritem no-label-item' : 'headeritem'">
                      ...
                      <div class="handle-tab-btn">
                        ...
                        <i class="el-icon-copy-document copy-item-first" @click.stop="copyFormsItem(ci, cinx)"></i>
                        <el-popconfirm title="确定删除吗？" @confirm="del(ci)">
              ...
              <div v-else class="empty-tab-item"></div>
            </template>
          </el-tab-pane>
        </el-tabs>
      </el-form>
      ...
      if(needPlaceFormula) {
        copyFormulaComponent(this.jvsAppId, this.designId, copy).then(res => {
          if(res.data && res.data.code == 0) {
            copy = res.data.data
            if(index > -1) {
              this.data.column[this.data.activeName].splice(index, 0, copy)
            }else{
              this.data.column[this.data.activeName].push(copy)
            }
            this.$nextTick(() => {
              this.set(item)
            })
