      ...
      <div class="step-bar-content" v-for="ti in data.dicData" :key="'step-bar-content'+ti.name" v-if="ti.name == tabkey">
        <el-row :gutter="10">
          <el-col v-for="(ci, cinx) in data.column[ti.name]" :key="ci.prop" :span="ci.span">
            <div
              :class="{'tab-item-form-item': true, 'tab-item-form-item-active': (actid === ci.prop && actCom && actCom.id == ci.id)}"
              :style="{'background': startindexTabItem == cinx ? '1px dashed #3a88ed' : endindexTabitem == cinx ? '1px dashed #3a88ed' :'', border: (actid === ci.prop && actCom && actCom.id == ci.id) ? '1px solid #3a88ed' : ''}"
              draggable
            ...
            >
              ...
              <div :class="['box', 'tableForm', 'formbox', 'reportTable'].indexOf(ci.type) === -1 ? 'headeritem no-label-item' : 'headeritem'">
                ...
                >
                  <!-- 一般项 -->
                  <!-- ,"box","link","iframe" -->
                  <FormItem  :item='ci' v-if='["tableForm", "formbox", "divider","p","tab","checkbox","tableReadOnly"].indexOf(ci.type) == -1 ' :designId="designId" />
                  <FormItem :form='dynamicValidateForm[ti.value]' :item='ci' v-if='ci.type === "checkbox"' :designId="designId" />
                </el-form-item>
                <el-form-item
                  style="flex:1;"
                  label-width="90px"
                  ...
                  :rules="ci.rules"
                >
                  <FormItem style="flex:1;"  :item='ci' :designId="designId" />
                </el-form-item>
                <!-- 分割线 -->
      ...
      if(bool) {
        ...
        if (['tab', 'step'].indexOf(this.com.type) == -1 ) {
          ...
          if(this.data.addcolumn){
            ...
            this.com.parentKey = this.data.parentKey ? (this.data.parentKey + '.') : '' + this.data.prop + '.' + this.tabkey
