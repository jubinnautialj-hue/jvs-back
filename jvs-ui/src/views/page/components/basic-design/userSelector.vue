...
<!--                <span style="font-size: 12px; color: #a2a3a5;">抱歉，没有找到相关搜索内容！</span>-->
          ...
          <!-- 系统字段 -->
          <el-tab-pane v-if="systemEnable && systemFields && systemFields.length > 0" label="系统字段" name="system">
            <div v-if="publicLoading" class="tree-loading-back"/>
            <div style="height: 360px;overflow: auto;margin-top: 10px;">
              <ul class="user-list-box userlist-box-tool">
                <li v-for="(item, index) in systemFields" :key="'check-field-'+index" @click="handleClickNode(item, 'system_field')">
                  <p>
                    {{item.name}}
                  </p>
                </li>
          ...
          <!-- 模型字段 -->
          <el-tab-pane v-if="systemEnable && formFields && formFields.length > 0" label="模型字段" name="form">
            <div v-if="publicLoading" class="tree-loading-back"/>
            <div style="height: 360px;overflow: auto;margin-top: 10px;">
              <ul class="user-list-box userlist-box-tool">
                <li v-for="(item, index) in formFields" :key="'check-form-'+index" @click="handleClickNode(item, 'model_field')">
                  <p>
                    {{item.name}}
                  </p>
                </li>
...
<script>
import {getSelectedUsers, getUserListPage, getUserTreeType, getNOticeUserOption} from "@/api/common";
export default {
  name: 'user-selector',
  props: {
    // 是否可选岗位
  ...
  methods: {
    ...
    handleClickNode(node, type) {
      ...
      if(['system_field', 'model_field'].indexOf(type) > -1) {
        name = node.name
    ...
    initCheckList(arr) {
      ...
      arr.forEach(item => {
        ...
        if (item.type === 'system_field') {
          this.checkList.push({ id: item.id, name: item.name, typeName: '系统字段' })
        }
        if(item.type === 'model_field') {
          this.checkList.push({ id: item.id, name: item.name, typeName: '模型字段' })
        }
      })
      if (userIds.length > 0) {
        ...
        if(ulist && ulist.length > 0) {
          ...
          arr.filter( item => {
            ...
            if (item.type === 'system_field') {
              olist.push({ id: item.id, name: item.name, typeName: '系统字段' })
            }
            if (item.type === 'model_field') {
              olist.push({ id: item.id, name: item.name, typeName: '模型字段' })
            }
          })
    ...
    // 获取系统字段
    getSystemFields() {
      getNOticeUserOption(this.jvsAppId, this.dataModelId).then(res => {
        if(res.data && res.data.code == 0 && res.data.data) {
          if(res.data.data.system_field && res.data.data.system_field.length > 0) {
            this.systemFields = res.data.data.system_field
          }
          if(res.data.data.model_field && res.data.data.model_field.length > 0) {
            this.formFields = res.data.data.model_field
          }
        }
      })
    }
