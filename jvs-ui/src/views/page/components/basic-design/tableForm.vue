<template>
  <div class="table-form">
    <jvs-table :key="regKey" :option="options" :data="tableData">
      ...
      <template v-for="(item, index) in options.tableColumn" :slot="item.prop" slot-scope="scope">
        <div :key="item.prop+'node'+index" style="width: 100%;">
          <div v-if="item.text && item.text.label && scope.row[item.prop] == item.text.value" :key="item.prop+'text'+index">
            <span>{{item.text.label}}</span>
          </div>
          <div v-if="item.needSlot !== true && !(item.text && item.text.label && scope.row[item.prop] == item.text.value) && displayByBind(item, scope.row)" :key="item.prop+'item'" style="width: 100%;">
            <!-- 文本类型 -->
            <el-input
              ...
              @input="valueChange(item, scope.row, scope.index)"
              size="mini"
              style="width: 100%;"
            ></el-input>
            <!-- 下拉选择 -->
          ...
          <div v-if="displayByBind(item, scope.row) && !(item.text && item.text.label && scope.row[item.prop] == item.text.value) && item.needSlot === true" :key="item.prop+'needslotitem'" style="width: 100%;">
...
export default {
  ...
  methods: {
    ...
    valueChange (item, row, index) {
      ...
      if (row.aliasColumnName && !this.regErr) {
