<template>
  <div class="permission-page">
    <div class="permission-body">
      ...
      <p>功能权限是指在表单上的对应操作按钮的权限。<span @click="handleMore('permission-help')">了解更多</span></p>
      <p>例如表单上新增数据的控制、删除数据的控制。</p>
          ...
          <el-form label-width="80px" label-position="top">
            ...
            <el-form-item label="操作权限" v-if="item.personType != 'all'">
              <el-checkbox-group v-model="item.operation">
                <el-checkbox v-for="(op, opKey) in operationList" :key="opKey" :label="op">{{ op }}</el-checkbox>
                <el-checkbox label="查看">查看</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
            <el-form-item v-if="false" label="数据权限">
...
<!--          <i v-if="key !== 0" class="el-icon-delete del-btn" @click="deleteGroupItem(item, key)"></i>-->
...
export default {
  ...
  methods: {
    ...
    // 表单数据初始化
    async initData(type, data) {
      this.loading = true
      if(type && data) {
        this.roleTypeTemp = type
        this.permissionList = (data.length > 0) ? this.getRole([...data]) : [{
          operation: [],
          personType: 'all',
          personnels: [],
          scopeList: [],
