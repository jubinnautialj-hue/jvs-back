<template>
  ...
  <div class="form-show-info">
    ...
    <div v-if="data.viewJson && permission">
      <jvs-form class="show-form" v-if="(formType == 'normalForm' || formType == 'detailForm') && showForm" :option="formOption" :formData="formData" :designId="$route.query.id || ''" @submit="formSubmit">
        <!-- 自定义按钮 -->
...
<!--        </template>-->
      ...
      <div v-if="formType == 'flowable'">
        <jvs-form class="show-form" :option="basicOption" @submit="formSubmit" :formData="formData.basic" :designId="$route.query.id || ''" @formChange="formChange">
          <!-- 自定义按钮 -->
        ...
        <jvs-form class="show-form" :option="formOption" @submit="formSubmit" :formData="formData.form" :designId="$route.query.id || ''" @formChange="formChange">
          <!-- 自定义按钮 -->
          <template slot="formButton" v-if="formOption.flag">
            <jvs-button size="mini" v-for="(item, index) in formOption.btnSetting" :key="item.name+'slotbtn'+index" @click="slotbtnClickHandle(item, index)">{{item.name}}</jvs-button>
          </template>
    ...
    formSubmit (formsdata) {
      ...
      if(this.isFlowable) {
        ...
        const http = {
          ...
          url: dataModelId ? `/mgr/jvs-design/app/use/${this.$route.query.jvsAppId}/dynamic/data/save/${dataModelId}` : `/mgr/jvs-design/app/use/${this.$route.query.jvsAppId}/dynamic/data/save`,
          headers: {
            'designId': designId,
    ...
    printHandle () {
      getPlugins('jvs-design').then(res =>{
        if(res.data && res.data['print-ui-url']) {
          let header = JSON.stringify({designId: this.id})
          let url = `${res.data['print-ui-url']}` + "jvs-print-ui/#/jvsPrint?"+ `access_token=${this.$store.getters.access_token}&`+ `identity=${this.id}&` + `access_fields=/mgr/jvs-design/app/use/${this.$route.query.jvsAppId}/dynamic/data/fields/${this.$route.query.dataModelId}&`
          if(this.data.viewJson.formdata && this.data.viewJson.formdata[0] && this.data.viewJson.formdata[0].formsetting && this.data.viewJson.formdata[0].formsetting.dataEchoRequest) {
            url += (`access_data=/mgr/jvs-design/app/use/${this.$route.query.jvsAppId}/rule/run/${this.data.viewJson.formdata[0].formsetting.dataEchoRequest}&` + `access_header=${header}&`)
            url += `access_method=post`
          }
