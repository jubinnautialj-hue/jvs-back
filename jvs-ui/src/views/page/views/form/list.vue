...
export default {
  ...
  methods: {
    ...
    async getList (page) {
      ...
      getPageList(this.$route.query.jvsAppId, Object.assign(obj, this.queryParam), paramStr).then(res => {
    ...
    addNewPage (form) {
      if(this.menuOpType == 'add') {
        ...
        addForm(this.$route.query.jvsAppId, form).then(res => {
          if(res.data.code == 0) {
      ...
      if(this.menuOpType == 'edit') {
        this.pageFormOption.submitLoading = true
        editForm(this.$route.query.jvsAppId, form).then(res => {
          if(res.data.code == 0) {
    ...
    undeployFormHandle (row) {
      if(row.id) {
        this.$confirm('卸载后所有的人无法操作，是否确定？', '提示', {
          ...
          undeployForm(this.$route.query.jvsAppId, row.id).then(res => {
            if(res.data.code == 0) {
    ...
    delRowHandle (row) {
      this.$confirm("删除后会导致引用了表单页的页面无法正常访问，是否继续删除？", '提示', {
        ...
        delForm(this.$route.query.jvsAppId, row.id).then(res => {
          if(res.data.code == 0) {
    ...
    exportFormHandle (row) {
      this.$openUrl(`/mgr/jvs-desig/app/${this.$route.query.jvsAppId}/form/design/export/${row.id}`, '_blank')
    },
    // 显示状态
    getIsDeployText(val) {
      return val ? '已发布' : '未发布'
    ...
    designForm (row) {
      if (row.id) {
        let str = ''
        str = location.origin + (`/page-design-ui/#/form?jvsAppId=${this.$route.query.jvsAppId}&id=`+row.id + (row.dataModelId ? `&dataModelId=${row.dataModelId}` : ''))
    ...
    async getKeyValueHandle () {
      await getKeyValue().then(res => {
        if(res.data.code == 0) {
          this.$store.commit('SET_LabelValue', res.data.data)
        }
      })
    },
  ...
  async created () {
    if(!this.$store.getters.labelValue) {
