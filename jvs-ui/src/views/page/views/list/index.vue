    ...
    >
      <template slot="menuLeft">
        ...
        <el-upload
          ...
          :action="`/mgr/jvs-design/app/${$route.query.jvsAppId}/design/import`"
          :multiple="false"
          :limit="1"
          ref="uploadBtn"
          :file-list="fileList"
...
export default {
  ...
  methods: {
    ...
    getList (page) {
      ...
      console.log(Object.assign(obj, this.queryParam))
    ...
    addRowHandle (form) {
      ...
      createPage(row.jvsAppId, form).then(res => {
        if(res.data.code == 0) {
    ...
    delRowHandle (row) {
      ...
      this.$confirm(str, '提示', {
        ...
        deletePage(row.jvsAppId, row.id).then(res => {
          if(res.data.code == 0) {
    ...
    designPage (row) {
      let str = ''
      str = location.origin + (`/page-design-ui/#/crud/design?jvsAppId=${row.jvsAppId}&id=`+row.id + (row.dataModelId ? `&dataModelId=${row.dataModelId}` : ''))
    ...
    addNewPage (form) {
      if(this.menuOpType == 'add') {
        this.pageFormOption.submitLoading = true
        createPage(form.jvsAppId, form).then(res => {
          if(res.data.code == 0) {
            // this.$message.success("新增页面成功")
            this.$notify({
              title: '提示',
      ...
      if(this.menuOpType == 'edit') {
        this.pageFormOption.submitLoading = true
        editPage(form.jvsAppId, form).then(res => {
          if(res.data.code == 0) {
            // this.$message.success("修改成功")
            this.$notify({
              title: '提示',
    ...
    publishPage (row) {
      ...
      if(msg) {
        this.$confirm(msg, '提示', {
          ...
          deployPage(row.jvsAppId, obj).then(res => {
            if(res.data.code == 0) {
    ...
    unPublishPage (row) {
      this.$confirm('卸载后所有的人无法操作，是否确定？', '提示', {
        ...
        unloadPage(row.jvsAppId, row.id).then(res => {
          if(res.data.code == 0) {
