<template>
  <div class="group-manage-box">
    <div class="group-manage">
      ...
      >
        <template slot="menuLeft">
          <jvs-button type="primary" size="mini" @click="dialogVisibleShow('add', null, null, 'post')" permisionFlag="jvs_group_add">{{$langt('group.addGroup')}}</jvs-button>
        </template>
        <template slot="menu" slot-scope="scope">
          <jvs-button type="text" size="mini" v-if="groupId !== ''" permisionFlag="jvs_group_delete_user" @click="delUSerGroup(scope.row)">{{$langt('group.remove')}}</jvs-button>
        </template>
            ...
            <span class="more-icon">
              <el-popover
                ...
                <ul class="base-type-list">
                  <li v-if="$permissionMatch('jvs_group_add_user')" @click.stop="addUser(data)">
                    <span>{{$langt('group.addMember')}}</span>
                  </li>
                  <li v-if="$permissionMatch('jvs_group_edit') && (!(data.childList && data.childList.length > 0))" @click.stop="() => dialogVisibleShow('edit', node, data, 'post')">
                    <span>{{$langt('table.edit')}}</span>
                  </li>
                  <li v-if="$permissionMatch('jvs_group_delete') && (!(data.childList && data.childList.length > 0))" @click.stop="() => remove(data)">
    ...
    <userSelector
      ...
      :selectable="true"
      :dialogTitle="$langt('group.userSelect')"
      @submit="addCheckUSer"
    />
  </div>
</template>
...
export default {
  name: 'group-manage',
  components: {PlatformPageHeader, dataPermision, userSelector},
  data () {
    return {
      groupId: '', // 团队id
      queryParams: {},
  ...
  created () {
    ...
    this.option.column.filter(item => {
      item.span=24
      if(item.label) {
        item.label = this.$langt(`group.column.${item.prop}.label`)
      }
    })
    this.formOption.column.filter(item => {
      ...
      if(item.rules && item.rules.length > 0) {
        item.rules[0].message = this.$langt(`group.column.${item.prop}.placeholder`)
      }
    })
    this.groupFormOption.column.filter(col => {
      if(col.label) {
        col.label = this.$langt(`group.column.${col.prop}.label`)
      }
      if(col.rules && col.rules.length > 0) {
        col.rules[0].message = this.$langt(`group.column.${col.prop}.placeholder`)
      }
    })
  ...
  methods: {
      ...
      getGroupList().then(res => { // this.selectOneId
        if(res.data && res.data.code == 0) {
          ...
          if(this.postData.length > 0) {
            this.groupId = this.postData[0].id
          }
    ...
    dialogVisibleShow (method, node, data, type) {
      ...
      this.postForm = {}
      if (method == 'add') {
        this.title = this.$langt('group.addGroup')
      }else{
        this.title = this.$langt('group.editGroup')
    ...
    remove (data) {
      this.$confirm(this.$langt('group.removeConfirm'), this.$langt('common.tip'), {
        ...
        deleteGroup(data.id).then(res => {
          if(res.data.code == 0) {
            this.$notify({
              ...
              message: this.$langt('group.delGroupSuccess'),
      ...
      if(this.method == 'add') {
        addGroup(obj).then(res => {
          if(res.data.code == 0) {
            this.$notify({
              title: this.$langt('common.tip'),
              message: this.$langt('group.addGroupSuccess'),
              position: 'bottom-right',
              type: 'success'
            });
      ...
      if(this.method == 'edit') {
        editGroup(obj).then(res => {
          if(res.data.code == 0) {
            this.$notify({
              title: this.$langt('common.tip'),
              message: this.$langt('group.editGroupSuccess'),
              position: 'bottom-right',
              type: 'success'
            });
            this.groupFormOption.submitLoading = false
    ...
    delUSerGroup (row) {
      this.$confirm(this.$langt('group.removeUserConfirm'), this.$langt('common.tip'), {
        confirmButtonText: this.$langt('common.confirm'),
        cancelButtonText: this.$langt('common.cancel'),
        type: 'warning'
      }).then(() => {
        ...
        removeUserByGroup(row.id, this.groupId).then(res => {
          if(res.data.code == 0) {
            this.$notify({
              title: this.$langt('common.tip'),
              message: this.$langt('group.delUserSuccess'),
              position: 'bottom-right',
              type: 'success'
            });
            this.getList()
    ...
    addCheckUSer (list) {
      ...
      editUserGroup(this.groupId, arr).then(res => {
        if(res.data.code == 0) {
          this.$notify({
            title: this.$langt('common.tip'),
            message: this.$langt('group.addUserSuccess'),
            position: 'bottom-right',
            type: 'success'
          });
