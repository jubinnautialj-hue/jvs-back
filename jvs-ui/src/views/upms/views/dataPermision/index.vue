  ...
  <div class="data-permision">
    ...
    >
      <template slot="menuLeft">
        <jvs-button size="mini" type="primary" @click="addHandle(null)" v-if="from != 'user'">{{$langt('table.add')}}</jvs-button>
      </template>
      <template slot="menu" slot-scope="scope">
        <jvs-button size="mini" type="text" @click="addHandle(scope.row)">{{$langt('table.edit')}}</jvs-button>
      </template>
    ...
    <el-dialog
      ...
      <jvs-form :option="option" :formData="addForm" @submit="addSubmit">
        <template slot="customForm">
          ...
          <jvs-button size="mini" @click="addOne" style="margin-top:10px;">{{$langt('table.plus')}}</jvs-button>
        </template>
        <template slot="deptForm">
          <div class="treebox" style="display: flex; align-items: center;height:36px;">
            <el-radio-group v-model="addForm.dept" size="mini">
              <el-radio label="all">{{$langt(`dataPermision.column.dept.dicData.all`)}}</el-radio>
              <el-radio label="oneself">{{$langt(`dataPermision.column.dept.dicData.oneself`)}}</el-radio>
              <el-radio label="subordinate">{{$langt(`dataPermision.column.dept.dicData.subordinate`)}}</el-radio>
              <el-radio label="custom">{{$langt(`dataPermision.column.dept.dicData.custom`)}}</el-radio>
            </el-radio-group>
            <div
              class="tree-auth"
...
export default {
  ...
  data () {
    return {
      ...
      addEditTitle: this.$langt('table.add')
    }
  },
  methods: {
    getList (page) {
    ...
    addHandle (row) {
      if(row) {
        this.addEditTitle = this.$langt('table.edit')
        ...
        this.addEditTitle = this.$langt('table.add')
      }
    ...
    addSubmit (form) {
      ...
      if(this.addEditTitle == this.$langt('table.add')) {
  ...
  created () {
    this.option.column.filter(col => {
      if(col.label) {
        col.label = this.$langt(`dataPermision.column.${col.prop}.label`)
      }
      if(col.dicData && col.dicData.length > 0) {
        col.dicData.filter(dit => {
          dit.label = this.$langt(`dataPermision.column.${col.prop}.dicData.${dit.value}`)
        })
      }
    })
    this.customOption.column.filter(col => {
      if(col.label) {
        col.label = this.$langt(`dataPermision.column.${col.prop}.label`)
      }
    })
