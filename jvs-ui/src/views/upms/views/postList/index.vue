<template>
  <div class="post-manage-box">
    <div class="post-manage">
      ...
      >
        <template slot="menuLeft">
          <jvs-button type="primary" size="mini" @click="dialogVisibleShow('add', null, null, 'post')" permisionFlag="jvs_job_add">{{$langt('postList.postAdd')}}</jvs-button>
        </template>
        <template slot="menu" slot-scope="scope">
          <jvs-button type="text" size="mini" v-if="jobId !== ''" permisionFlag="jvs_job_delete_user" @click="delUSerPost(scope.row)"><span style="color: #F56C6C;">{{$langt('table.remove')}}</span></jvs-button>
        </template>
      ...
      <div class="treeBox post-treeBox">
        <div :class="{'treeBox-title': true, 'treeBox-title-check': !jobId}" @click="queryAllHandle">{{$langt('common.all')}}</div>
        <el-tree
          ref="postTree"
          :data="postData"
          node-key="id"
            ...
            <span class="more-icon">
              <el-popover
                ...
                <ul class="base-type-list">
                  <li v-if="$permissionMatch('jvs_job_add_user')" @click.stop="addUser(data)">
                    <!-- <i class="el-icon-delete iconhover"></i> -->
                    <span>{{$langt('postList.memberAdd')}}</span>
                  </li>
                  <li v-if="$permissionMatch('jvs_job_edit') && (!(data.children && data.children.length > 0))" @click.stop="() => dialogVisibleShow('edit', node, data, 'post')">
                    <!-- <i class="el-icon-setting iconhover"></i> -->
                    <span>{{$langt('table.edit')}}</span>
                  </li>
                  <li v-if="$permissionMatch('jvs_job_delete') && (!(data.children && data.children.length > 0))" @click.stop="() => remove(data)">
                    <!-- <i class="el-icon-delete iconhover"></i> -->
                    <span>{{$langt('table.delete')}}</span>
                  </li>
                </ul>
      ...
      <div v-if="dialogVisible">
        <jvs-form v-if="formType == 'post'" :option="postFormOption" :formData="postForm" @submit="postSubmitHandle" @cancalClick="handleClose">
          <template slot="datapermisionForm">
            <span>{{$langt('postList.dataDesc')}}</span>
          </template>
          <template slot="dataScopeTypeForm">
            <div class="permision-radio" style="display: flex; align-items: center;height:auto;min-height:36px;flex-wrap: wrap;padding-bottom: 10px;">
              <el-radio-group v-model="postForm.dataScopeType" size="mini" style="width: 100%;">
                <el-radio label="all_dept">{{$langt('common.all_dept')}}</el-radio>
                <el-radio label="curr_dept">{{$langt('common.curr_dept')}}</el-radio>
                <el-radio label="curr_dept_tree">{{$langt('common.curr_dept_tree')}}</el-radio>
                <el-radio label="all_job">{{$langt('common.all_job')}}</el-radio>
                <el-radio label="curr_job">{{$langt('common.curr_job')}}</el-radio>
                <el-radio label="self">{{$langt('common.self')}}</el-radio>
                <el-radio label="customize">{{$langt('common.customize')}}</el-radio>
              </el-radio-group>
              <div
                class="tree-auth"
              ...
              >
                <el-cascader
                  v-model="deptIds"
                  :placeholder="$langt('postList.deptIds')"
                  size="mini"
                  :options="treedata"
                  clearable
                ...
                <el-select v-model="jobIds" :placeholder="$langt('postList.jobIds')" size="mini" multiple :collapse-tags="true" style="margin-left:10px;">
                  <el-option
          ...
          <template slot="deptIdForm">
            <el-cascader
              ...
              :options="treedata"
              clearable
              :disabled="title == $langt('postList.postAdd') ? false : true"
              :show-all-levels="false"
              :props="{
                expandTrigger: 'hover',
                checkStrictly: true,
    ...
    <userSelector
      ...
      :selectable="true"
      :dialogTitle="$langt('postList.userSelect')"
      @submit="addCheckUSer"
    />
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    this.option.column.filter(col => {
      if(col.label) {
        col.label = this.$langt(`postList.column.${col.prop}.label`)
      }
      if(col.tips && col.tips.text) {
        col.tips.text = this.$langt(`postList.column.${col.prop}.tip`)
      }
      if(col.rules && col.rules.length > 0) {
        col.rules[0].message = this.$langt(`postList.column.${col.prop}.placeholder`)
      }
    })
    this.formOption.column.filter(item => {
      item.span=24
      if(item.label) {
        item.label = this.$langt(`postList.column.${item.prop}.label`)
      }
      if(item.rules && item.rules.length > 0) {
        item.rules[0].message = this.$langt(`postList.column.${item.prop}.placeholder`)
      }
    })
    this.postFormOption.column.filter(col => {
      if(col.label) {
        col.label = this.$langt(`postList.column.${col.prop}.label`)
      }
      if(col.tips && col.tips.text) {
  ...
  methods: {
    ...
    getList (page) {
      ...
      if(this.queryParams.current !== this.page.currentPage) {
        this.queryParams.current = this.page.currentPage
      }  
    ...
    dialogVisibleShow (method, node, data, type) {
      ...
      if(type == 'post') {
        ...
        this.postForm = {}
        if (method == 'add') {
          this.title = this.$langt('postList.postAdd')
          this.postForm = {
            dataScopeType: "all_dept"
          }
        }else{
          this.title = this.$langt('postList.postEdit')
      ...
      if(type == 'user'){
        this.title = this.$langt('postList.userPosEdit')
        this.rowData = JSON.parse(JSON.stringify(data))
      }
      this.dialogVisible = true
    },
    remove (data) {
      this.$confirm(this.$langt('postList.delPosConfirm'), this.$langt('common.tip'), {
        ...
        delPost(data.id).then(res => {
          if(res.data.code == 0) {
            this.$notify({
              ...
              message: this.$langt('postList.delPosSuccess'),
      ...
      if(this.method == 'add') {
        addPost(obj).then(res => {
          if(res.data.code == 0) {
            this.$notify({
              title: this.$langt('common.tip'),
              message: this.$langt('postList.addPosSuccess'),
              position: 'bottom-right',
              type: 'success'
            });
      ...
      if(this.method == 'edit') {
        editPost(obj).then(res => {
          if(res.data.code == 0) {
            this.$notify({
              title: this.$langt('common.tip'),
              message: this.$langt('postList.editPosSuccess'),
              position: 'bottom-right',
              type: 'success'
            });
            this.postFormOption.submitLoading = false
    ...
    delUSerPost (row) {
      this.$confirm(this.$langt('postList.delUserPosConfirm'), this.$langt('common.tip'), {
        confirmButtonText: this.$langt('common.confirm'),
        cancelButtonText: this.$langt('common.cancel'),
        type: 'warning'
      }).then(() => {
