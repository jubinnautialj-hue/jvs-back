...
<!--  </el-dialog>-->
  <el-dialog
    width="760px"
    class="invitation-box-dialog"
    :title="$langt('department.invite')"
    :visible.sync="dialogVisible"
    :close-on-click-modal="false"
    :before-close="handleClose">
    <div class="invitation-box" v-if="dialogVisible">
      <div class="invitation-header">
        <img v-if="$store.state.user.userInfo.tenant.logo" :src="$store.state.user.userInfo.tenant.logo" alt="">
        <div class="header-text">
          <div>{{$store.state.user.userInfo.tenant.shortName}}</div>
          <div style="color: #a2a3a5">{{$langt('department.inviteTip')}}</div>
        </div>
      </div>
      <div class="invitation-body">
        <div class="left">
          <div class="left-header">
            <div style="color: #333333;margin-bottom: 6px;">{{$langt('department.inviteCode')}}</div>
            <div style="color: #a2a3a5">{{$langt('department.inviteDesc')}}</div>
          </div>
          ...
          <div class="invite-btn">
            <el-button
              ...
              v-clipboard:error="onError"
              type="primary"
              size="mini">{{$langt('department.inviteCopy')}}</el-button>
          </div>
        </div>
        <div class="right">
          <div style="padding: 20px 20px 15px">{{$langt('department.inviteTime')}}</div>
          <div style="padding: 15px 20px;display: flex;align-items: center;">
            <el-button type="text" @click="getInviteCode">{{$langt('department.inviteRep')}}</el-button>
            <svg class="help-entry" aria-hidden="true">
              <use xlink:href="#icon-help"></use>
            </svg>
          </div>
          <div style="padding: 15px 20px">{{$langt('department.inviteInfo')}}</div>
...
export default {
  ...
  methods: {
    ...
    onError (e) {
      this.$notify({
        title: this.$langt('common.tip'),
        message: this.$langt('common.copyFail'),
        position: 'bottom-right',
        type: 'warning'
      });
      console.log(e)
