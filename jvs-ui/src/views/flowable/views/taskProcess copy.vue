        ...
        <div :class="{'task-form-body': true, 'task-form-body-hideRight': hideProcessList}">
          <div class="left-part">
            ...
            <div :class="{'form-info': true, 'form-info-fullscreen': fullscreenBool}" :style="taskFormType == 'view' ? 'padding-bottom: 0;' : ''">
              <showForm v-if="!isOut && formShow" :formUrl="formUrl" :rowData="rowData" :btn="btn" :flowDesign="flowDesign" :defaultFormData="defaultFormData" :btnHide="taskFormType == 'view' ? true : false" :onlyView="taskFormType == 'view' ? true : false" :extendInfo="taskDataInfo.extend" :enableSign="enableSign" @close="closeHandle" toHome />
            </div>
          </div>
...
export default {
  ...
  data(){
    return {
      ...
      rowData: {},
      taskFormType: '',
      showTaskForm: !(/Android|webOS|iPhone|iPad|BlackBerry/i.test(navigator.userAgent)),
    }
  },
  watch: {
    rowData: {
      handler(rowData) {
        if(rowData && rowData.jvsAppId && !this.showTaskForm) {
  ...
  methods: {
    ...
    getParamsFromUrl() {
      return new Promise((resolve) => {
        this.taskId = this.$route.query.processId;
         let temp= {}
        temp.taskId = this.taskId
        let query={}
    queryTaskList(Object.assign(temp, query)).then(res => {
      if(res.data.code == 0 && res.data.data.records) {
            ...
            taskRecordList(Object.assign(temp, query)).then(resdata => {
                      if (resdata.data.code==0) {
                        this.rowData = resdata.data.data.records[0];
                        this.$nextTick(async () => {
                          await this.queryDetailHistoryInfoHandle();
                          await this.getEchoFormData();
                          if(this.taskDataInfo.nodes && this.taskDataInfo.nodes.length > 0 && this.taskDataInfo.nodes[0].formId) {
      ...
      if (this.rowData && obj.id === this.rowData.nodeId) {
        if (obj.id === '10000') {
          ...
          if(this.taskDataInfo.nodes && this.taskDataInfo.nodes.length > 0) {
            this.taskDataInfo.nodes.filter(nit => {
              if(nit.nodeId == obj.id) {
                if(nit.approvalType == 2 && obj.props && obj.props.appendApproval) { // 添加obj.props检查
                  isNodeBtn = false
          ...
          if(buttons && buttons.length > 0) {
            if(obj.props && obj.props.enableSign) { // 添加obj.props检查
