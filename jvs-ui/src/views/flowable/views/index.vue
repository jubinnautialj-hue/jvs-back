  ...
  <div class="flowable-index">
    ...
    >
      ...
      <template slot="published" slot-scope="scope">
        <el-tag size="mini" :type="scope.row.published ? '' : 'info'">{{scope.row.published ? '已发布' : '未发布'}}</el-tag>
      </template>
        ...
        <jvs-form v-if="dialogType == 'edit' || dialogType == 'add'" style="padding: 0 80px;" :option="formOption" :formData="rowData" @submit="modelInfoSubmit">
          <template slot="iconForm">
            <div class="icon-image">
              <img v-if="rowData && rowData.icon" :src="rowData.icon" alt="">
              <el-button v-if="rowData && !rowData.icon" size="mini" @click="handleFocus">选择图标</el-button>
              <i v-if="rowData && rowData.icon" class="el-icon-delete" @click="rowData.icon = ''"></i>
            </div>
          </template>
        </jvs-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    addRowHandle (form) {
      ...
      if(!temp.extend) {
        temp.extend = {
          ...
          enableRestart: form.enableRestart
        }
      }else{
        this.$set(temp.extend, 'enableCancel', form.enableCancel)
        this.$set(temp.extend, 'enableRestart', form.enableRestart)
      }
      ...
      if(row) {
        ...
        if(type == 'edit') {
          if(row.extend) {
            ...
            this.$set(row, 'enableRestart', row.extend.enableRestart)
          }
        }
        // 设计
        if(type == 'design') {
    ...
    getDetail (row) {
      // 跳转设计
      this.$openUrl(`/flowable-ui/#/processDesign?id=${row.id}&jvsAppId=${row.jvsAppId}`, '_blank')
    },
    // 删除
    deleteRowHandle (row) {
      this.$confirm('确认删除？').then(_ => {
        deleteModel(row.jvsAppId, row.id).then(res => {
          if(res.data.code == 0) {
            this.$notify({
              title: '提示',
              message: '删除成功',
    ...
    deployHandle (row) {
      this.$confirm('确认发布流程？', '提示', {
        ...
        deployProcess(row.jvsAppId, row.id).then(res => {
          if(res.data.code == 0) {
            this.$notify({
              title: '提示',
              message: '发布成功',
    ...
    suspendHandle (row) {
      this.$confirm('确认挂起流程？').then(_ => {
        suspendProcess(row.jvsAppId, row.id).then(res => {
          if(res.data.code == 0) {
            this.$notify({
              title: '提示',
              message: '流程挂起成功',
