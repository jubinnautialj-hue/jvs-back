  ...
  <div class="cont">
    ...
    >
      ...
      <template slot="enable" slot-scope="scope">
        <el-tag size="mini" :type="scope.row.enable ? '' : 'info'">{{scope.row.enable ? '已启用' : '已禁用'}}</el-tag>
      </template>
    ...
    <el-dialog class="ljlog-no-header-dialog" title="日志" v-if="logVisible" :visible.sync="logVisible" :fullscreen="true" :close-on-click-modal="false" :before-close="handleClose">
    ...
    <el-dialog :visible.sync="jsonVisible" width="50%" append-to-body :close-on-click-modal="false" :before-close="jsonClose">
...
<script>
import { getList, delEdition, editJSON, SavaJSON, pageLog } from '../api/rule' //  jsonExport, enable
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      getList(this.$route.query.jvsAppId, { ...obj, ...this.queryForm}).then(res => {
        if(res.data.code == 0) {
          this.loading=false
    ...
    del (row) {
      this.$confirm('此操作将永久删除该设计,是否继续?', '提示', {
        ...
        delEdition(row.jvsAppId, row.id).then(res => {
    ...
    bianj (row) {
      SavaJSON().then(res => {
        if(res.data.code == 0) {
          this.getData()
          this.$openUrl(`/rule-design-ui/#/ruleDesign?id=${res.data.data}&name=未命名逻辑`, '_blank')
        }
      })
      // if(row) {
    ...
    See (row) {
      ...
      this.$openUrl(`/rule-design-ui/#/ruleDesign?id=${row.secret}&name=${row.name}&jvsAppId=${row.jvsAppId}`, '_blank')
    },
    // 查看日志
    viewLogHandle (row) {
      if(row) {
    ...
    viewLog () {
      ...
      if(row) {
        pageLog(row.jvsAppId, row.secret, {current: this.logPage.currentPage, size: this.logPage.pageSize}).then(res => {
          if(res.data.code == 0) {
    ...
    editHandle (obj) {
      ...
      editJSON(row.jvsAppId, obj.id, temp).then(res => {
        if(res.data.code == 0) {
