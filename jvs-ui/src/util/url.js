...
export default {
  install (Vue, options) {
    // 打开链接 用于 预览、下载、打开地址
    Vue.prototype.$openUrl=function (url, type, dictKey, openBy) {
      if (dictKey) {
      ...
      if (url) {
        ...
        // 文件流下载 导出
        if(url.startsWith('/mgr') && !url.includes('print') && openBy != 'url') {
          ...
          if(window.navigator.userAgent.includes('DingTalk') && typeTemp == '_blank') {
            if(url.includes('?')) {
              url+= '&ddtab=true'
            }else{
              url+= '?ddtab=true'
