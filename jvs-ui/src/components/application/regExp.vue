<template>
  <div class="regexp-list">
    ...
    <el-dialog
      ...
      <jvs-form v-if="formVisible" ref="myform" :option="option" :formData="rowData" @submit="submitForm">
...
<script>
import {getRegexpList, typeRegList, addReg, editReg, delReg} from '@/api/application'
function isReg(reg) {
  let isReg;
  try {
...
export default {
  ...
  methods: {
    getList (page) {
      ...
      getRegexpList(Object.assign({}, tp, this.queryParams)).then(({ data }) => {
    ...
    gettypeRegListHandle () {
      typeRegList().then(res => {
        if(res.data.code == 0) {
