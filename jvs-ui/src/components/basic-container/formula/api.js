...
// 根据变量中心类别名称获取全部函数
export function getFuncList(requestPrefix, useCase, id, extendJson) {
  let obj = {
  ...
  if(useCase) {
    ...
    if(extendJson) {
      obj.params.extendJson = extendJson
    }
  }
  return request(obj);
}
...
// 批量删除公式
export function mulDeleteExec(designId, data) {
  return request({
    url: `/mgr/jvs-design//jvsFunction/exec/${designId}`,
    method: "put",
    data: data
  });
}
...
// 解析函数
export function jiexiExec(data) {
  return request({
    url: `/mgr/jvs-design/jvsFunction/test/parse`,
    method: "put",
    data: data
  });
}
...
export function testExec(data) {
  return request({
    url: `/mgr/jvs-design/jvsFunction/test/exec`,
    method: "put",
    data: data
  });
}