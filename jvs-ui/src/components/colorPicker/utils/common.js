 ...
 * @returns 
 */
export function getBackImgPos(val) {
  let styleStr = ''
  if (val?.pos) {
    let posArr = val.pos.split('-')
    if (posArr.length == 1) {
    ...
    styleStr += `background-position: center center;`
  }
  return styleStr
}
 ...
 * @returns 
 */
export function getBackImgStyle(val) {
  let styleStr = 'width: 100%;height: 100%;background-size: contain;background-repeat: no-repeat;background-blend-mode: normal;'
  if (val.backImg) {
    styleStr += `background-image:url(${val.backImg});`
  }
  styleStr += `transform: rotate(${val.rotate}deg)`
  if (val.horizontalFlip) {
    styleStr += ` scaleX(-1)`
  }
  if (val.verticalFlip) {
    styleStr += ` scaleY(-1)`
