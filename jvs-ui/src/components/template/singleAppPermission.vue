<template>
  <div class="single-app-permission-page">
    ...
    <div class="content-box">
      ...
      <div class="left-type-list">
        <div v-for="(sort, sindex) in groupShowSort" :key="sort" class="left-type-list-item">
          <div class="group-title">
            <h4>{{getGroupLabel(sindex)}}</h4>
            <i class="el-icon-circle-plus-outline" @click="addGroupItem(sort)"></i>
          </div>
          <div class="group-list">
            <div class="el-tree">
              <div v-for="data in groupTypeList" :key="'role-item-'+data.id" v-if="data.permissionType == sort" :class="{'el-tree-node ': true, 'is-current': (currentId == data.id)}">
      ...
      <div v-if="rowData" class="right-box">
        ...
        <div class="top-box">
          <div style="display: flex;align-items: center;justify-content: space-between;">
            <div>
              <el-radio-group v-if="rowData.permissionType == 'OPERATION'" v-model="rowData.member.personType" size="mini" style="margin-right: 20px;">
                <el-radio label="all">所有人</el-radio>
                <el-radio label="custom">自定义</el-radio>
              </el-radio-group>
              <el-button v-if="rowData.permissionType == 'OPERATION' ? (rowData.member.personType != 'all') : true" size="mini" type="primary" icon="el-icon-plus" class="add-button" @click="handleUserSelectOpen">选择人员/角色/部门</el-button>
            </div>
            <div>
        ...
        <div class="bottom-box">
          ...
          <div class="right-permission-list">
            <!-- 功能权限组 -->
            <div v-if="rowData.permissionType == 'OPERATION'" class="permission-table">
              <div class="table-title">
                <div class="title-item" style="width: 160px;box-sizing: border-box;">分类</div>
                <div class="title-item" style="width: calc(100% - 214px);box-sizing: border-box;">功能权限</div>
                <div style="width: 50px;box-sizing: border-box;margin-right: 4px;">
              ...
              <div class="table-body">
                ...
                <div class="table-body-item" v-for="(menu, key) in designDataListTemp" :key="key">
                  <div class="category common-border">{{ menu.name }}</div>
                  <div class="object">
                    <!-- 操作权限 -->
                    <div class="object-box">
                      <div v-if="menu.designType == 'page'" class="object-item-list object-item-title">
                        <div>操作权限</div>
                      </div>
                      <div class="object-item-list" style="border-bottom: 0;">
                        <div class="object-item" v-for="(obj, objKey) in menu.operation" :key="'oprate-item-'+key+'-'+objKey">
                          <div class="object-item-name">
                            <span :class="{'check-label': true, 'isCheck': obj.check}" @click="obj.check = !obj.check;handleCheckChange(menu);">
                    ...
                    <!-- 目录权限 -->
                    <div v-if="menu.designType == 'page'" class="object-box">
                      <div class="object-item-list object-item-title">
                        <div>目录权限</div>
                      </div>
                      <div class="object-item-list">
                        <div class="object-item" v-for="(obj, objKey) in menu.treeOperation" :key="'left-tree-item-'+key+'-'+objKey">
                          <div class="object-item-name">
                            <span :class="{'check-label': true, 'isCheck': obj.check}" @click="obj.check = !obj.check;handleCheckChange(menu);">
                              <span class="check"></span>
                              <span class="label">{{obj.name}}</span>
                    ...
                    <!-- 关联的权限设置 -->
                    <div v-for="(ritem, rix) in menu.relevant" :key="'relation-item-'+rix" class="object-box">
                      <!-- 关联名称 -->
                      <div class="object-item-list object-item-title">
                        <div style="color: #333;">{{ritem.name}}</div>
                      </div>
                      <!-- 操作权限 -->
                      <div v-if="ritem.designType == 'page'" class="object-item-list object-item-title">
                        <div>操作权限</div>
                      </div>
                      <div class="object-item-list" style="border-bottom: 0;">
                        <div class="object-item" v-for="(obj, objKey) in ritem.operation" :key="'relation-oprate-item-'+key+'-'+rix+'-'+objKey">
                          <div class="object-item-name">
                            <span :class="{'check-label': true, 'isCheck': obj.check}" @click="obj.check = !obj.check;handleCheckChange(menu);">
                              <span class="check"></span>
                              <span class="label">{{obj.name}}</span>
                      ...
                      <div v-if="ritem.designType == 'page'" class="object-item-list">
                        <div class="object-item" v-for="(obj, objKey) in ritem.treeOperation" :key="'relation-left-tree-item-'+key+'-'+rix+'-'+objKey">
                          <div class="object-item-name">
                            <span :class="{'check-label': true, 'isCheck': obj.check}" @click="obj.check = !obj.check;handleCheckChange(menu);">
                              <span class="check"></span>
                              <span class="label">{{obj.name}}</span>
            ...
            <div v-if="rowData.permissionType == 'DATA'" class="permission-table">
              <div class="table-title">
                <div class="title-item" style="width: 160px;box-sizing: border-box;">分类</div>
                <div class="title-item" style="width: calc(100% - 160px);border-right: 0;;box-sizing: border-box;">功能权限</div>
              </div>
              <div class="table-body">
                <div v-if="permissionLoading" class="loading-back"/>
...
import { getAppMenuPermissionList, getAppPageList, getAppPermissionGroupList, addAppPermissionGroup, editAppPermissionGroup, delAppPermissionGroup, saveAppPermissionGroup, getAppPermissionByGroup, upgradeVersion } from './api'
import { getDataModelDataFilter} from "@/views/page/api/list";
export default {
  name: 'singleAppPermission',
  ...
  methods: {
    getAppPermissionGroupList () {
      if(this.appInfo && this.appInfo.id) {
        getAppPermissionGroupList(this.appInfo.id, 'DESIGN').then(res => {
          if(res.data && res.data.code == 0) {
            ...
            if(this.groupTypeList && this.groupTypeList.length > 0) {
              ...
              this.groupTypeList.filter(git => {
                ...
                if(!fir) {
                  if(git.permissionType == 'OPERATION') {
                    fir = git
                  }
                }
              })
              if(!fir) {
                fir = this.groupTypeList[0]
              }
              if(!hasIn) {
                this.$nextTick(() => {
                  this.groupHandleClick(fir)
                })
              }
            }
          }
    ...
    async getDesignData () {
      if(this.appInfo && this.appInfo.id) {
        await getAppMenuPermissionList(this.appInfo.id).then(res => {
          if(res.data && res.data.code == 0) {
            this.designDataList = res.data.data
    ...
    getPageList () {
      if(this.appInfo && this.appInfo.id) {
        getAppPageList(this.appInfo.id).then(res => {
          if(res.data && res.data.code == 0) {
    ...
    formatDesignData () {
      this.designDataListTemp = []
      for(let i in this.designDataList) {
        let obj = {}
        if(this.designDataList[i].id) {
          obj = {
            id: this.designDataList[i].id,
            name: this.designDataList[i].name,
            designType: this.designDataList[i].designType
          }
        }
        // 操作权限
        obj.operation = []
        if(this.designDataList[i].operation && this.designDataList[i].operation.length > 0) {
          this.designDataList[i].operation.filter(fit => {
            obj.operation.push({
              name: fit,
              check: false
            })
        ...
        // 目录权限
        if(this.designDataList[i].treeOperation && this.designDataList[i].treeOperation.length > 0) {
          obj.treeOperation = []
          this.designDataList[i].treeOperation.filter(fit => {
            obj.treeOperation.push({
              name: fit,
              check: false
            })
          })
        ...
        //   }
        // }
        if(this.designDataList[i].relevant && this.designDataList[i].relevant.length > 0) {
          obj.relevant = []
          for(let j in this.designDataList[i].relevant) {
            let tob = {
              id: this.designDataList[i].relevant[j].id,
              name: this.designDataList[i].relevant[j].name,
              designType: this.designDataList[i].relevant[j].designType
            }
            tob.operation = []
            if(this.designDataList[i].relevant[j].operation && this.designDataList[i].relevant[j].operation.length > 0) {
              this.designDataList[i].relevant[j].operation.filter(fit => {
                tob.operation.push({
                  name: fit,
                  check: false
                })
            ...
            if(this.designDataList[i].relevant[j].treeOperation && this.designDataList[i].relevant[j].treeOperation.length > 0) {
              tob.treeOperation = []
              this.designDataList[i].relevant[j].treeOperation.filter(fit => {
                tob.treeOperation.push({
                  name: fit,
                  check: false
                })
              })
            ...
            obj.relevant.push(tob)
          }
        }
        this.designDataListTemp.push(obj)
      }
    },
    formatDataPermissionData () {
      ...
      this.pageList.filter(item => {
        ...
        this.dataPermissionData.push({...item, dataPermissionList: tp}) // stepDataPermission: false,
      })
      this.$forceUpdate()
    },
    setDesignTempData (bool) {
      for(let i in this.designDataListTemp) {
        this.$set(this.designDataListTemp[i], 'isIndeterminate', bool)
        this.$set(this.designDataListTemp[i], 'check', bool)
        if(this.designDataListTemp[i].operation) {
          this.designDataListTemp[i].operation.filter(fit => {
            this.$set(fit, 'isIndeterminate', bool)
            this.$set(fit, 'check', bool)
          })
        }
        if(this.designDataListTemp[i].treeOperation) {
          this.designDataListTemp[i].treeOperation.filter(fit => {
            this.$set(fit, 'isIndeterminate', bool)
            this.$set(fit, 'check', bool)
          })
        }
        if(this.designDataListTemp[i].relevant) {
          for(let j in this.designDataListTemp[i].relevant) {
            this.$set(this.designDataListTemp[i].relevant[j], 'isIndeterminate', bool)
            this.$set(this.designDataListTemp[i].relevant[j], 'check', bool)
            if(this.designDataListTemp[i].relevant[j].operation) {
              this.designDataListTemp[i].relevant[j].operation.filter(fit => {
                this.$set(fit, 'isIndeterminate', bool)
                this.$set(fit, 'check', bool)
              })
            }
            if(this.designDataListTemp[i].relevant[j].treeOperation) {
              this.designDataListTemp[i].relevant[j].treeOperation.filter(fit => {
                this.$set(fit, 'isIndeterminate', bool)
                this.$set(fit, 'check', bool)
              })
            }
    ...
    getGroupLabel (index) {
      let str = ''
      switch(index) {
        case 0: str = '功能权限组';break;
        case 1: str = '数据权限组';break;
        default: ;break;
      }
      return str
    },
    ...
    groupHandleClick (data, node, dom) {
      if(data) {
        if(this.currentId != data.id) {
          ...
          if(data.permissionType == 'OPERATION') {
            if(!this.rowData.member) {
              this.$set(this.rowData, 'member', {personType: 'all', personnels: []})
            }
            this.formatDesignData()
          }
    ...
    // 所有权限全选
    checkAllChange (e) {
      this.setDesignTempData(e)
    },
    // 行数据全选
    handleCheckAllChange (obj) {
      if(obj.operation) {
        obj.operation.filter(item => {
          this.$set(item, 'check', obj.check)
        })
      }
      if(obj.treeOperation) {
        obj.treeOperation.filter(item => {
          this.$set(item, 'check', obj.check)
        })
      }
      if(obj.relevant) {
        for(let i in obj.relevant) {
          if(obj.relevant[i].operation) {
            obj.relevant[i].operation.filter(item => {
              this.$set(item, 'check', obj.check)
            })
          }
          if(obj.relevant[i].treeOperation) {
            obj.relevant[i].treeOperation.filter(item => {
              this.$set(item, 'check', obj.check)
            })
          }
        }
    ...
    handleCheckChange(obj) {
      ...
      let arr = []
      if(obj.operation) {
        arr = arr.concat(obj.operation)
      }
      if(obj.treeOperation) {
        arr = arr.concat(obj.treeOperation)
      }
      if(obj.relevant) {
        for(let i in obj.relevant) {
          if(obj.relevant[i].operation) {
            arr = arr.concat(obj.relevant[i].operation)
          }
          if(obj.relevant[i].treeOperation) {
            arr = arr.concat(obj.relevant[i].treeOperation)
          }
        }
      }
      arr.filter(item => {
    ...
    setFieldCondition (item) {
      ...
      this.getDataModelFilterList(item.dataModelId)
    },
    ...
    saveCurrentPermission () {
      let temp = {
        member: {
          personType: (this.rowData.member && this.rowData.member.personType) ? this.rowData.member.personType : 'custom',
          personnels: this.rowData.member ? this.rowData.member.personnels : []
        },
        permissions: []
      }
      if(this.rowData.permissionType == 'OPERATION') {
        for(let i in this.designDataListTemp) {
          let tob = {
            designId: this.designDataListTemp[i].id,
            permission: {}
          }
          if(this.designDataListTemp[i].operation && this.designDataListTemp[i].operation.length > 0) {
            this.$set(tob.permission, 'operation', [])
            this.designDataListTemp[i].operation.filter(oit => {
              if(oit.check) {
                tob.permission.operation.push(oit.name)
              }
            })
          }
          if(this.designDataListTemp[i].treeOperation && this.designDataListTemp[i].treeOperation.length > 0) {
            this.$set(tob.permission, 'treeOperation', [])
            this.designDataListTemp[i].treeOperation.filter(oit => {
              if(oit.check) {
                tob.permission.treeOperation.push(oit.name)
              }
            })
          }
          if((tob.permission.operation && tob.permission.operation.length > 0) || (tob.permission.treeOperation && tob.permission.treeOperation.length > 0)) {
            temp.permissions.push(tob)
          }
          if(this.designDataListTemp[i].relevant && this.designDataListTemp[i].relevant.length > 0) {
            this.designDataListTemp[i].relevant.filter(rit => {
              let tobj = {
                designId: rit.id,
                permission: {}
              }
              if(rit.operation && rit.operation.length > 0) {
                this.$set(tobj.permission, 'operation', [])
                rit.operation.filter(oit => {
                  if(oit.check) {
                    tobj.permission.operation.push(oit.name)
                  }
                })
              }
              if(rit.treeOperation && rit.treeOperation.length > 0) {
                this.$set(tobj.permission, 'treeOperation', [])
                rit.treeOperation.filter(oit => {
                  if(oit.check) {
                    tobj.permission.treeOperation.push(oit.name)
                  }
                })
              }
              if((tobj.permission.operation && tobj.permission.operation.length > 0) || (tobj.permission.treeOperation && tobj.permission.treeOperation.length > 0)) {
                temp.permissions.push(tobj)
              }
            })
          }
        }
    ...
    showAlreadyCheck (list) {
      if(list && list.length > 0) {
        if(this.rowData.permissionType == 'OPERATION') {
          for(let i in this.designDataListTemp) {
            list.filter(lit => {
              if(this.designDataListTemp[i].id == lit.designId) {
                if(lit.permission) {
                  if(lit.permission.operation && lit.permission.operation.length > 0) {
                    for(let j in this.designDataListTemp[i].operation) {
                      if(lit.permission.operation.indexOf(this.designDataListTemp[i].operation[j].name) > -1) {
                        this.$set(this.designDataListTemp[i].operation[j], 'check', true)
                      }
                    }
                  }
                  if(lit.permission.treeOperation && lit.permission.treeOperation.length > 0) {
                    for(let j in this.designDataListTemp[i].treeOperation) {
                      if(lit.permission.treeOperation.indexOf(this.designDataListTemp[i].treeOperation[j].name) > -1) {
                        this.$set(this.designDataListTemp[i].treeOperation[j], 'check', true)
                      }
                    }
                  }
                }
              ...
              if(this.designDataListTemp[i].relevant && this.designDataListTemp[i].relevant.length > 0) {
                for(let r in this.designDataListTemp[i].relevant) {
                  if(this.designDataListTemp[i].relevant[r].id == lit.designId) {
                    if(lit.permission) {
                      if(lit.permission.operation && lit.permission.operation.length > 0) {
                        for(let j in this.designDataListTemp[i].relevant[r].operation) {
                          if(lit.permission.operation.indexOf(this.designDataListTemp[i].relevant[r].operation[j].name) > -1) {
                            this.$set(this.designDataListTemp[i].relevant[r].operation[j], 'check', true)
                          }
                        }
                      }
                      if(lit.permission.treeOperation && lit.permission.treeOperation.length > 0) {
                        for(let j in this.designDataListTemp[i].relevant[r].treeOperation) {
                          if(lit.permission.treeOperation.indexOf(this.designDataListTemp[i].relevant[r].treeOperation[j].name) > -1) {
                            this.$set(this.designDataListTemp[i].relevant[r].treeOperation[j], 'check', true)
                          }
                        }
                      }
                    }
