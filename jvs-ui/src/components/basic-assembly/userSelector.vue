<template>
  <el-dialog
    ...
    top="12vh"
    :width="isRadio ? ((userEnable && !(personnelScopes && personnelScopes.length > 0) && !rangeOptionAble) ? '720px' :'420px') : '720px'"
    class="user-info-list-dialog"
  >
    <div :class="{'user-info-list': true, 'just-user-info-list': (userEnable && !(personnelScopes && personnelScopes.length > 0) && !rangeOptionAble)}" :style="(isRadio || !isSingleTab) ? 'height: 420px;' : 'height: 460px;'" v-if="dialogVisible">
      <div :class="isSingleTab ? 'left dept-treeBox' : 'left dept-treeBox single-left'" :style="isRadio ? 'border-right: none' : ''">
        <div class="description-box">
          <p>* 支持用户名、手机号搜索，不支持帐号搜索</p>
          <p v-if="!allEnable">* 已删除的用户无法搜索</p>
          <p>* 默认展示前100条</p>
        </div>
        <el-tabs size="mini" v-model="activeName" @tab-click="handleTabClick">
          <el-tab-pane v-if="userEnable" label="人员" name="user">
            <el-input size="mini" style="width: 100%;" v-model="keyword" @input="searchUserHandle" clearable placeholder="输入手机号或名字搜索" prefix-icon="el-icon-search"></el-input>
            <div v-if="userLoading" class="tree-loading-back"/>
            <div class="user-choose" style="height: 270px;overflow: auto;margin-top: 10px;">
              <div v-if="!(personnelScopes && personnelScopes.length > 0) && !rangeOptionAble" class="user-choose-left">
                <div class="user-choose-left-top">
                  <p :class="{'active': userByActive == 'dept'}" @click="changeUserByActive('dept')">按部门</p>
                  <p :class="{'active': userByActive == 'job'}" @click="changeUserByActive('job')">按岗位</p>
                </div>
                <div class="user-choose-left-content">
                  <div v-show="userByActive == 'dept'">
                    <el-tree
                      ...
                      node-key="id"
                      @node-click="deptSearchUser"
                      ref="userByDeptTree">
                    </el-tree>
                  </div>
                  <div v-if="userByActive == 'job'">
                    <ul class="user-list-box userlist-box-tool">
                      <li v-for="(item, index) in jobList" :key="'check-user-'+index" :class="{'job-active': userBySearch == item.id}" @click="jobSearchUser(item)">
                        <p>{{item.name}}</p>
                      </li>
                    </ul>
                  </div>
              ...
              <div v-if="!isRadio" style="width: 1px; background-color: #eee;margin-left: 20px" />
              <div class="user-choose-right">
          ...
          <el-tab-pane v-if="roleEnable" label="角色" name="role">
            ...
            <div style="height: 270px;overflow: auto;margin-top: 10px;">
              <ul class="user-list-box userlist-box-tool">
      ...
      <div v-if="!isRadio" class="right">
        ...
        <div class="right-header">
          ...
          <el-button style="color: #F56C6C" type="text" @click="handleClear">清空</el-button>
        </div>
        <div class="right-list" :style="(isSingleTab ? 'height: 410px;' : 'height: 370px;') + 'overflow: auto;margin-top: 10px;'">
          <ul class="user-list-box">
            <li v-for="(item, index) in checkList" :key="'check-user-'+index">
              <el-popover placement="bottom" trigger="hover" :content="item.typeName ? '（' + item.typeName + '）' + item.name : item.name" popper-class="p custom-right-tool-poper">
                <p class="head-name" slot="reference">
                  <img v-if="item.headImg" :src="item.headImg" alt="">
                  <span>{{item.typeName ? '（' + item.typeName + '）' + item.name : item.name}}</span>
                </p>
              </el-popover>
              <p>
    ...
    <div v-if="!isRadio" class="footer-btn">
      <div style="font-size: 12px;color: #868BA1;">共计：(<span>{{ allCount }}</span>)</div>
      <div>
...
<script>
import {getSelectedUsers, getUserListPage, getUserTreeType, getUserListPageByFlow} from "@/api/common";
export default {
  name: 'user-selector',
  props: {
    // 是否可选岗位
  ...
  computed: {
    rangeOptionAble() {
      return (this.rangeOption && this.rangeOption.sortable)
    }
  },
  data () {
    return {
  ...
  watch: {
    activeName(val) {
      this.getAllCount(val)
    }
  },
  async created() {
    this.$nextTick(() => {
      this.$refs.userSelector.$el.addEventListener('click', e => {
        e.stopPropagation()
      })
    })
  },
  methods: {
    getAllCount(val) {
      switch (val) {
        case 'user':
          this.allCount = this.page.total;
          break;
        case 'role':
          this.allCount = this.roleList.length;
          break;
        case 'group':
          this.allCount = this.groupList.length;
          break;
        case 'dept':
          this.allCount = this.deptList.length;
          break;
        case 'job':
          this.allCount = this.jobList.length;
          break;
        default: this.allCount = this.page.total;break;
      }
    },
    // 上一页
    lastPage() {
    ...
    handleDeptNodeClick(data) {
      const obj = { id: data.id, name: data.name, typeName: '部门', namePath: data.parentNameList }
    ...
    async openDialog (arr, personnelScopes) {
      this.isFinish = false
      ...
      if(personnelScopes) {
        this.personnelScopes = personnelScopes
      }else{
        this.personnelScopes = null
      }
      await this.initCheckList(list)
      await this.getUserList()
      await this.getTreeData()
      this.getAllCount(this.currentActiveName)
      this.isFinish = true
    },
    // 初始化已选数据
    initCheckList(arr) {
      ...
      if(userIds.length > 0 && (this.userEnable && !this.isSingleTab)) {
        ...
        if(ulist && ulist.length > 0) {
          ...
          let tempArr = []
          ulist.filter(uit => {
            tempArr.push({ id: uit.id, name: uit.name, type: 'user' })
          })
          this.checkData = [...tempArr]
        }else{
          let olist = []
          let tempArr = []
          arr.filter( item => {
            tempArr.push({ id: item.id, name: item.name, type: item.type === 'user' ? 'user' : item.type })
            if(item.type === 'user') {
    ...
    submit () {
      ...
      this.$emit('submit', submitArr, this.checkList)
      this.closeDialog()
    },
    // 取消
    cancel () {
    ...
    async getUserList () {
      ...
      let params = {
        ...
        all: this.allEnable == true ? true : false
      }
      if(this.personnelScopes && this.personnelScopes.length > 0) {
        params.personnelScopes = this.personnelScopes
        await getUserListPageByFlow(params).then(res => {
          if(res.data && res.data.code == 0) {
        ...
        if(this.userBySearch) {
          if(this.userByActive == 'dept') {
            params.deptId = this.userBySearch
          }
          if(this.userByActive == 'job') {
            params.jobId = this.userBySearch
          }
        }
        if(this.userEnable && this.rangeOptionAble) {
          let ids = []
          this.rangeOption.scopes.filter(rit => {
            ids.push(rit.id)
          })
          params.rangType = this.rangeOption.type
          params.rangIds = ids
        }
    ...
    async getTreeData () {
      ...
      if(this.deptEnable && this.rangeOptionAble) {
        let ids = []
        this.rangeOption.scopes.filter(rit => {
          ids.push(rit.id)
        })
      ...
      await getUserTreeType(params).then(res => {
        if(res.data && res.data.code == 0) {
          ...
          this.eachDeptData(this.deptList)
    ...
    changeUserByActive (type) {
      this.userByActive = type
      this.userBySearch = ''
      if(this.$refs.userByDeptTree) {
        this.$refs.userByDeptTree.setCurrentKey()
      }
      this.searchUserHandle()
    },
    // 根据部门查询用户
    deptSearchUser (data) {
      this.userBySearch = data.id
      this.searchUserHandle()
    },
    // 根据岗位查询用户
    jobSearchUser (item) {
      this.userBySearch = item.id
      this.searchUserHandle()
    },
    eachDeptData (list, parent) {
      for(let i in list) {
        ...
        if(list[i].childList && list[i].childList.length > 0) {
          this.eachDeptData(list[i].childList, list[i])
        }
      }
    }
  },
