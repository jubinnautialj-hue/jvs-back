      ...
      <div class="dialog-box-content">
        ...
        <div class="categories-box">
          ...
          <div v-for="(item, key) in labelList" :key="key">
            <div :tabindex="key" class="categories-item" :style="queryParams.label === item ? 'background-color: #f7f8fd;color: #3471ff;' : ''" @click="handleClickItem(item, key)">{{ item }}</div>
          </div>
        </div>
        <div class="image-list-box">
          ...
          <div v-for="(item, key) in imageList" :key="key">
            <div class="image-item">
              <div v-if="false" class="search-icon">
                <i class="el-icon-search" @click="handlePreview(item)"/>
              </div>
              <div v-if="false && $permissionMatch('jvs_file_delete')" class="delete-icon">
              ...
              <div class="img-btn">
                <div class="download-btn" @click="handlePreview(item)">预览</div>
                <el-popover
              ...
              <!--            <img src="@/const/img/noPermission.jpg"/>-->
            </div>
...
<script>
import {getImageList, getLabelList, deleteFile} from "../api";
  ...
  methods: {
    ...
    init () {
      if(this.paramInfo) {
        ...
        this.$set(this.paramData, 'module', this.paramInfo.module || 'chart')
        this.$set(this.paramData, 'label', this.paramInfo.label || '背景')
        this.$set(this.queryParams, 'label', this.paramInfo.label || '背景')
      }
    },
    // 文件上传成功
    ...
    beforeUpload(file) {
      ...
      const isPassType = (file.type !== png && file.type !== jpg && file.type !== gif && file.type !== ico)
      const isLt2M = file.size / 1024 / 1024 < 50;
      if (isPassType && !isLt2M) {
        // this.$message.error('上传文件的格式只能是 png、jpg、jpeg、gif 格式，且文件大小不能超过 50MB！');
        this.$notify({
          title: '提示',
          message: '上传文件的格式只能是 png、jpg、jpeg、gif、ico 格式，且文件大小不能超过 50MB！',
        ...
        this.$notify({
          title: '提示',
          message: '文件格式错误，仅支持上传png、jpg、jpeg、gif、ico格式文件!',
          position: 'bottom-right',
          type: 'error'
        });
      } else if (!isLt2M) {
    ...
    async getImages() {
      ...
      const params = {
        ...
        size: 10,
        bucketName: (this.paramInfo && this.paramInfo.bucketName) ? this.paramInfo.bucketName : 'jvs-public',
        fileType: 'image'
      }
  ...
  .dialog-box-content{
    ...
    .image-list-box{
      ...
      .image-item{
        ...
        background-image:
          linear-gradient(45deg, #e0e0e0 25%, transparent 0, transparent 75%, #e0e0e0 0),
          linear-gradient(45deg, #e0e0e0 25%, transparent 0, transparent 75%, #e0e0e0 0);
        ...
        &:hover{
          ...
          .search-icon, .delete-icon, .img-btn{
            opacity: 1;
          }
        }
      }
