...
import { loginByUsername, refreshToken, codeLogin } from "@/api/login";
...
const user = {
  ...
  actions: {
        ...
        if (user.loginType === "namepass") {
          loginByUsername(
            ...
            .then(response => {
              ...
              commit("SET_FUNCTIONS_OBJ", data.license);
              resolve(data);
            })
            .catch(error => {
              // console.log(error);
    ...
    LogOut({ commit }, type) {
      ...
      commit("SET_FUNCTIONS_OBJ", {});
      commit("SET_MODEUSER_INFO", {});
      commit("SET_AUTO_OPENED_DICT", []);
  ...
  mutations: {
    ...
    SET_USER_INFO: (state, userInfo) => {
      ...
      if(userInfo && userInfo.tenant && userInfo.tenant.systemName) {
        document.title = userInfo.tenant.systemName
      }
      setStore({
        name: "userInfo",
      ...
      if(userInfo.realName || userInfo.accountName) {
        warterMark.set(userInfo.realName || userInfo.accountName)
      }
    },
