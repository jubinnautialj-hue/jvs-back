...
export function updateLine(line, text, markedSpans, estimateHeight, change) {
  if(sessionStorage.getItem('formulaData')) {
    nodeDataList = JSON.parse(sessionStorage.getItem('formulaData'))
  }
  line.text = text
  if(change && change.nodeData) {
    line.nodeData = change.nodeData
    nodeDataList[change.nodeData.id] = change.nodeData
  }else{
    line.nodeData = null
  }
...
export function buildLineContent(cm, lineView) {
  ...
  if(lineView.line.nodeData) {
    nodeData = lineView.line.nodeData
  }else{
    nodeData = null
  }
  let content = eltP("span", null, null, webkit ? "padding-right: .1px" : null, nodeData);
...
function buildToken(builder, text, style, startStyle, endStyle, css, attributes) {
  ...
  if (!special.test(text)) {
    ...
    if(nodeData) {
      ...
      tel.innerHTML = ( '&ZeroWidthSpace;' + nodeData.name + '&ZeroWidthSpace;')
      tel.setAttribute("class", `cm-field ${(nodeData && nodeData.param) ? ('cm-field-param'+nodeData.parent) : 'cm-field-function'}`)
      tel.setAttribute("title", `${nodeData ? nodeData.id : ''}`)
      content = tel
    }else{
    ...
    while (true) {
      ...
      if (skipped) {
        let txt
        let ttxstr = displayText.slice(pos, pos + skipped)
        let karr = ttxstr.replace(/\$\{/g, "")
        karr = karr.replace(/\}/g, "")
        if((nodeData && ('${'+nodeData.id+'}') == ttxstr) || (nodeDataList[karr])) {
          // console.log('------------', ttxstr)
          txt = document.createElement('span')
          let dataTemp  = null
          if(nodeDataList[karr]) {
            dataTemp = nodeDataList[karr]
          }else{
            dataTemp  = nodeData
          }
          txt.innerHTML = dataTemp.name // ( '&ZeroWidthSpace;' + dataTemp.name + '&ZeroWidthSpace;')
          txt.setAttribute("class", `cm-field ${(dataTemp && dataTemp.param) ? ('cm-field-param'+dataTemp.parent) : 'cm-field-function'}`)
          txt.setAttribute("title", `${dataTemp ? dataTemp.id : ''}`)
        }else{
          // console.log('************', ttxstr)
          txt = document.createTextNode(ttxstr)
        }
