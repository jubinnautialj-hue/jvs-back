...
Doc.prototype = createObj(BranchChunk.prototype, {
  ...
  replaceSelection: function(code, collapse, origin, nodeData) {
    let dup = []
    for (let i = 0; i < this.sel.ranges.length; i++)
      dup[i] = code
    this.replaceSelections(dup, collapse, origin || "+input", nodeData)
  },
  replaceSelections: docMethodOp(function(code, collapse, origin, nodeData) {
    let changes = [], sel = this.sel
    for (let i = 0; i < sel.ranges.length; i++) {
      let range = sel.ranges[i]
      changes[i] = {from: range.from(), to: range.to(), text: this.splitLines(code[i]), origin: origin, nodeData: nodeData}
    }
