...
function filterChange(doc, change, update) {
  let obj = {
    ...
    nodeData: change.nodeData,
    cancel: () => obj.canceled = true
  }
  if (update) obj.update = (from, to, text, origin, nodeData) => {
    if (from) obj.from = clipPos(doc, from)
    if (to) obj.to = clipPos(doc, to)
    if (text) obj.text = text
    if (origin !== undefined) obj.origin = origin
    if(nodeData !== undefined) obj.nodeData = nodeData
  }
  ...
  return {from: obj.from, to: obj.to, text: obj.text, origin: obj.origin, nodeData: obj.nodeData}
}
...
export function makeChange(doc, change, ignoreReadOnly) {
  ...
  if (split) {
    for (let i = split.length - 1; i >= 0; --i)
      makeChangeInner(doc, {from: split[i].from, to: split[i].to, text: i ? [""] : change.text, origin: change.origin, nodeData: change.nodeData})
  } else {
...
function makeChangeSingleDoc(doc, change, selAfter, spans) {
  ...
  if (change.from.line < doc.first) {
    ...
    change = {from: Pos(doc.first, 0), to: Pos(change.to.line + shift, change.to.ch),
              text: [lst(change.text)], origin: change.origin, nodeData: change.nodeData}
  }
  let last = doc.lastLine()
  if (change.to.line > last) {
    change = {from: change.from, to: Pos(last, getLine(doc, last).text.length),
              text: [change.text[0]], origin: change.origin, nodeData: change.nodeData}
  }
