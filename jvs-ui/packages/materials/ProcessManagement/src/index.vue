<template>
  <div class="process-manage-cards-box" :style="getChartIconPos()" @click="viewHandle">
    ...
    <div class="card-right" :style="{alignItems: (componentSetting.contentAlign == 'left' ? 'start' : 'center')}">
      <div class="name" :style="{
        ...
        {{getLabel(componentSetting.componentName)}}
      </div>
      <div v-if="countInfo" class="content" :style="{
        ...
        {{countInfo[componentSetting.componentName] || 0}}
      </div>
    </div>
    <workplace v-show="false" ref="workplace" :formIndex="true" @getCount="getCount"></workplace>
  </div>
</template>

<script>
import workplace from '../../../../src/page/main/workplace/workplace'
import { tranRenderColor, getLastColor } from '@/components/colorPicker/utils/common'
import { processTypeList } from '../../../customForm/src/config'
export default {
  ...
  methods: {
    ...
    viewHandle () {
      let obj = {
        label: this.getLabel(this.componentSetting.componentName)
      }
      this.$refs.workplace.headerItemClick(obj)
    }
  }
}
</script>
