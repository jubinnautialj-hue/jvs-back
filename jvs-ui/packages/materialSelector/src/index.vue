<template>
    <div class="material-selector-wrapper">
        ...
        <div class="tips">{{$hit(activeItem.text)}}</div>
        </div>
        <el-button type="primary" :disabled="disabled" @click="handleOpenSelector" style="margin: 0">{{$hit('选择物料')}}</el-button>
        <custom-dialog
...
<script>
import { MATERIAL_LIST_MAP } from '@/const'
export default {
  ...
  computed:{
    activeItem(){
        return MATERIAL_LIST_MAP[this.value]
    }
  },
  mounted() {
    this.materialList = Object.keys(MATERIAL_LIST_MAP).map(key => {
    ...
    }).filter(item=>{return !item.hidden})
  },
  methods: {
    handleOpenSelector(){
