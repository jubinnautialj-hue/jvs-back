...
public class ApplyController {
    ...
    public R<Boolean> save(@RequestBody @Validated Apply apply) {
        long count = applyService.count(Wrappers.<Apply>lambdaQuery().eq(Apply::getName, apply.getName()).last(SysConstant.FOR_UPDATE));
        if (count > 0) {
    ...
    @CacheEvict(value = SysConstant.CACHE_CLIENT, allEntries = true)
