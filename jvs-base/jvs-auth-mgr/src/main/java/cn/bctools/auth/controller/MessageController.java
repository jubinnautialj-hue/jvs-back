...
public class MessageController {
    ...
    public R<Page<Message>> page(Page page, Message message) {
        messageService.page(page, Wrappers.<Message>lambdaQuery()
                //屏蔽内容
                .select(Message.class, e -> !e.getProperty().equals(Get.name(Message::getContent)))
                ...
                //收件人模糊查询
                .and(ObjectUtil.isNotEmpty(message.getRecipients()), wrapper -> wrapper.apply(SqlFunctionUtil.jsonContainsObject(Get.name(Message::getRecipients), "$", "name", message.getRecipients().get(0).toString())))
