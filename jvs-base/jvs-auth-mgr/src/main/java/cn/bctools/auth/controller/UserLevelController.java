...
public class UserLevelController {
    ...
    public R saveLevel(@RequestBody UserLevel userLevel) {
        //判断是否重复
        long count = userLevelService.count(Wrappers.query(new UserLevel().setName(userLevel.getName())));
        if (count > 0) {
    ...
    public R deleteLevel(@PathVariable String id) {
        long count = userTenantService.count(Wrappers.query(new UserTenant().setAccountLevelId(id)));
        if (count > 0) {
            return R.failed("用户等级下有用户不能删除");
        }
