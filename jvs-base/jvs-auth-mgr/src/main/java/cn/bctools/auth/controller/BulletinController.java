...
public class BulletinController {
    ...
    public R<String> create(@Validated @RequestBody Bulletin vo) {
        if (LocalDateTimeUtil.between(vo.getStartTime(), vo.getEndTime(), ChronoUnit.MILLIS) <= 0) {
            throw new BusinessException("开始时间必须大余结束时间");
        }
        Bulletin bulletin = BeanCopyUtil.copy(vo, Bulletin.class);
    ...
    public R<Page<Bulletin>> page(Page<Bulletin> page, Bulletin vo) {
        LambdaQueryWrapper<Bulletin> query = Wrappers.<Bulletin>lambdaQuery()
                .like(StringUtils.isNotBlank(vo.getKeywords()), Bulletin::getTitle, vo.getKeywords())
                .or(StringUtils.isNotBlank(vo.getKeywords()), e -> e.like(Bulletin::getContent, vo.getKeywords()))
    ...
    public R<List<Bulletin>> list() {
        ...
        LambdaQueryWrapper<Bulletin> query = Wrappers.<Bulletin>lambdaQuery()
                ...
                .apply(SqlFunctionUtil.jsonContains("app_keys", appKey, "$"))
