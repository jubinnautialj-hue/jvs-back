...
public class JobController {
    ...
    public R<List<Job>> list() {
        return R.ok(jobService.list(Wrappers.<Job>lambdaQuery().orderByAsc(Job::getCreateTime)));
    }

    @Log
    @ApiOperation(value = "岗位下的用户", notes = "岗位下面所有的用户")
    @GetMapping("/users")
    public R<Page<UserVo>> users(JobUserPageReqVo reqVo, PageDTO<UserTenant> page) {
        //中间关系
        userTenantService.page(page, Wrappers.<UserTenant>lambdaQuery()
                .eq(UserTenant::getCancelFlag, false)
                .eq(StringUtils.isNotBlank(reqVo.getJobId()), UserTenant::getJobId, reqVo.getJobId())
                .like(StringUtils.isNotBlank(reqVo.getRealName()), UserTenant::getRealName, reqVo.getRealName())
                .like(StringUtils.isNotBlank(reqVo.getPhone()), UserTenant::getPhone, reqVo.getPhone()));
    ...
    public R<Boolean> deleteUser(@PathVariable("userId") String userId) {
        userTenantService.update(Wrappers.<UserTenant>lambdaUpdate()
    ...
    public R<Boolean> putUser(@RequestBody List<UserTenant> list, @PathVariable("jobId") String jobId) {
    ...
    @Transactional(rollbackFor = Exception.class)
    public R<Boolean> job(@PathVariable("jobId") String jobId) {
        long count = userTenantService.count(Wrappers.query(new UserTenant().setJobId(jobId)));
        if (count > 0) {
            return R.failed("岗位下有用户不能删除");
        }
        // 删除部门信息
