...
public class UserApiImpl implements AuthUserServiceApi {
    ...
    public R<List<UserDto>> users() {
        ...
        // 用户对象转换
        return R.ok(BeanCopyUtil.copys(users, UserDto.class));
    }
    ...
    @Override
    public R<List<UserDto>> getByNames(List<String> userNames) {
        List<UserDto> collect = userService.list(new LambdaQueryWrapper<User>().in(User::getRealName, userNames))
                .stream()
                .map(e -> BeanCopyUtil.copy(e, UserDto.class)).collect(Collectors.toList());
        return R.ok(collect);
    }
    ...
    @Override
    public R<BaseFindReturnDto> baseSearch(BaseFindDto dto) {
        BaseFindReturnDto baseFindReturnDto = new BaseFindReturnDto();
        if (ObjectNull.isNotNull(dto.getDeptIds())) {
            List<Dept> list = deptService.list(new LambdaQueryWrapper<Dept>().in(Dept::getId, dto.getDeptIds()));
            List<SysDeptDto> copys = BeanCopyUtil.copys(list, SysDeptDto.class);
            baseFindReturnDto.setDeptDtoList(copys);
        }
        if (ObjectNull.isNotNull(dto.getUserIds())) {
            List<User> list = userService.list(new LambdaQueryWrapper<User>().in(User::getId, dto.getUserIds()));
            List<UserDto> copys = BeanCopyUtil.copys(list, UserDto.class);
            baseFindReturnDto.setUserDtoList(copys);
        }
        if (ObjectNull.isNotNull(dto.getGroupIds())) {
            List<UserGroup> list = userGroupService.list(new LambdaQueryWrapper<UserGroup>().in(UserGroup::getId, dto.getGroupIds()));
            List<UserGroupDto> copys = BeanCopyUtil.copys(list, UserGroupDto.class);
            baseFindReturnDto.setGroupDtoList(copys);
        }
        if (ObjectNull.isNotNull(dto.getRoleIds())) {
            List<Role> list = roleService.list(new LambdaQueryWrapper<Role>().in(Role::getId, dto.getRoleIds()));
            List<SysRoleDto> copys = BeanCopyUtil.copys(list, SysRoleDto.class);
            baseFindReturnDto.setRoleDtoList(copys);
        }
        if (ObjectNull.isNotNull(dto.getJobIds())) {
            List<Job> list = jobService.list(new LambdaQueryWrapper<Job>().in(Job::getId, dto.getJobIds()));
            List<SysJobDto> copys = BeanCopyUtil.copys(list, SysJobDto.class);
            baseFindReturnDto.setJobDtoList(copys);
        }
        return R.ok(baseFindReturnDto);
    }
    ...
    public R<Boolean> userDelete(String userId) {
        ...
        long count = userService.count(Wrappers.<User>lambdaQuery().eq(User::getId, userId));
        if (count == 0) {
            return R.failed("用户不存在");
        }
    ...
    public R<UserDto> getBasicInfoById(String id) {
        ...
        // 判断是否是超级管理员
        TenantPo thisTenantPo = tenantService.getById(tenantServiceOne.getTenantId());
        String adminUserId = thisTenantPo.getAdminUserId();
        UserDto userDto = BeanCopyUtil.copy(UserDto.class, user, tenantServiceOne);
        userDto.setId(id);
        userDto.setAdminFlag(id.equalsIgnoreCase(adminUserId));
        return R.ok(userDto);
    }

    @Override
    public R<List<UserDto>> getBasicInfoById(List<String> userIds) {
        List<User> users = userService.listByIds(userIds);
        List<UserDto> userDto = BeanCopyUtil.copys(users, UserDto.class);
        return R.ok(userDto);
    }

    @Override
    public R<List<UserDto>> getBasicInfoById(List<String> userIds, List<String> field) {
        List<User> users = userService.list(Wrappers.query(new User()).lambda().in(User::getId, userIds).select(User.class, e -> field.contains(e.getProperty())));
        List<UserDto> userDto = BeanCopyUtil.copys(users, UserDto.class);
        return R.ok(userDto);
    }
    ...
    private void clearUser(@NotNull String userId) {
        ...
        userRoleComponent.clearUser(userId);
