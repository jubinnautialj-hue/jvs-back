...
@Component
@AllArgsConstructor
public class MpTextMessageHandler extends BaseMessageHandler<TextMessageDTO> {
    ...
    public void handle(TextMessageDTO param) {
        ...
        SysConfigWx config = configServiceApi.convertKey(ConfigsTypeEnum.WECHAT_OFFICIAL_ACCOUNT_CONFIGURATION, body);
        if (!config.getEnable()) {
            throw new IllegalArgumentException("未找到微信公众号配置，请核实!");
        }

        WxMpService wxService = SingletonUtil.get(config.getAppKey() + config.getAppSecret(), () -> {
