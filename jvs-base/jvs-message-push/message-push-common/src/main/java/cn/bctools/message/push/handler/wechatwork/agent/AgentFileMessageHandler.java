...
@Component
@AllArgsConstructor
public class AgentFileMessageHandler extends BaseMessageHandler<MediaMessageDTO> {
    ...
    public void handle(MediaMessageDTO param) {
        ...
        //获取配置详情
        R<String> key = configServiceApi.key(ConfigsTypeEnum.ENTERPRISE_WECHAT_APPLICATION_CONFIGURATION, TenantContextHolder.getTenantId());
        SysConfigEnterriseWeChat config = configServiceApi.convertKey(ConfigsTypeEnum.ENTERPRISE_WECHAT_APPLICATION_CONFIGURATION, key.getData());
        if (!config.getEnable()) {
        ...
        param.getDefinedReceivers().forEach(e -> {
            ...
            MessagePushHis messagePushHis = new MessagePushHis()
                    ...
                    .setMessageType(MessageTypeEnum.WECHAT_WORK_AGENT_FILE);
