...
@Component
@AllArgsConstructor
public class TemplateMessageHandler extends BaseMessageHandler<TemplateMessageDTO> {
    ...
    private WxMpService createTemplateService() {
        String body = configServiceApi.key(ConfigsTypeEnum.WECHAT_OFFICIAL_ACCOUNT_CONFIGURATION, TenantContextHolder.getTenantId()).getData();
        SysConfigBase data = configServiceApi.convertKey(ConfigsTypeEnum.WECHAT_OFFICIAL_ACCOUNT_CONFIGURATION, body);
        if (!data.getEnable()) {
            throw new IllegalArgumentException("未找到微信公众号配置，请核实!");
        }
        config = BeanCopyUtil.copy(data,WechatOfficialAccountConfig.class);
        ...
        WxMpService wxService = SingletonUtil.get(config.getAppId() + config.getAppSecret(), () -> {
            ...
            mpConfig.setAppId(config.getAppKey());
    ...
    private WxMpTemplateMessage createTemplate(TemplateMessageDTO param, ReceiversDto receiverUser) {
        ...
        for (WechatTemplateData wechatTemplateData : templateDataList) {
            ...
            // 校正颜色
            color = StringUtils.isNoneBlank(color) && !color.contains("#") ? "#" + color : color;
    ...
    public List<WxMpTemplateVo> getAllTemplate() {
        ...
        //配合微信公众号新规
        result.forEach(e->{
            e.setContent(e.getContent().replaceAll("\\{\\{first\\.DATA}}",""));
            e.setContent(e.getContent().replaceAll("\\{\\{remark\\.DATA}}",""));
        });
        return result;
    }
}
