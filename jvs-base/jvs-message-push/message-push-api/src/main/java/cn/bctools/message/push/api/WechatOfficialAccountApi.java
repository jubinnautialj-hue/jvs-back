...
//    R<Boolean> resendWebChatMpTextMessage(String pushHisId);
    ...
    R<Boolean> sendWebChatTemplateMessage(@Validated @RequestBody TemplateMessageDTO messageDto);
     ...
     * @return
     */
    @GetMapping(PREFIX + "/template/all/{sync}")
    @ApiOperation("获取当前公众号的模板列表")
    List<WxMpTemplateVo> getAllPrivateTemplate(@PathVariable("sync") Boolean sync);
}
