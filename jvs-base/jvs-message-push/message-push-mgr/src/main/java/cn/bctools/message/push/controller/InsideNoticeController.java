...
import cn.bctools.common.utils.DateUtils;
...
public class InsideNoticeController {
    ...
    @GetMapping("/page")
    @ApiOperation("分页查询")
    public R<Page> page(Page<InsideNotice> page, InsideNotice dto) {
        ...
        Page<Object> objectPage = new Page<>(page.getCurrent(), page.getSize(), page.getTotal());
        List<Object> collect = page.getRecords().stream().map(e -> DateUtils.transformation(e, Get.name(InsideNotice::getCreateTime))).collect(Collectors.toList());
