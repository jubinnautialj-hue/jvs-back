...
public class DingSendUtils {
    ...
    public DingRes sendMessage(String content) {
        return sendMessage(dingDingConfig.getUrl(), dingDingConfig.getSecret(), content, dingDingConfig.getPhones());
    }
    ...
    public DingRes sendMessage(String webhook, String secret, String content, List<String> mobiles) {
        if (ObjectNull.isNull(secret)) {
            return null;
        }
        webhook = getUrl(webhook, secret);
        TestMessage testMessage = new TestMessage(content, mobiles);
        //添加默认超时时间
        String body = HttpUtil.post(webhook, JSONObject.toJSONString(testMessage),3000);
    ...
    public String getUrl(String url, String secret) {
        ...
        try {
            ...
            sign = URLEncoder.encode((Base64.encode(signData)), Charset.defaultCharset().name());
        } catch (Exception e) {
            log.error("加密出错", e);
        }
