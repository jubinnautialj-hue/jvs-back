...
public class DefaultRedisCacheWriter implements RedisCacheWriter {
    ...
    private JvsCacheConvert jvsCacheConvert;

    /**
     * Instantiates a new Default redis cache writer.
     *
     ...
     * @param keys              the keys
     * @param jvsCacheConvert   the jvs cache convert
     */
    public DefaultRedisCacheWriter(RedisConnectionFactory connectionFactory, BatchStrategy keys, JvsCacheConvert jvsCacheConvert) {
        this(connectionFactory, Duration.ZERO, keys);
        this.jvsCacheConvert = jvsCacheConvert;
    }
    ...
    public void clean(String name, byte[] pattern) {
        ...
        execute(name, connection -> {
            ...
            try {
                ...
                long deleteCount = batchStrategy.cleanCache(connection, finalName, pattern);
                while (deleteCount > Integer.MAX_VALUE) {
                    statistics.incDeletesBy(finalName, Integer.MAX_VALUE);
                    deleteCount -= Integer.MAX_VALUE;
                }
                statistics.incDeletesBy(finalName, (int) deleteCount);
    ...
    public CacheStatistics getCacheStatistics(String cacheName) {
        cacheName = jvsCacheConvert.cacheName(cacheName);
    ...
    void lock(String name) {
        String finalName = name;
        execute(name, connection -> doLock(finalName, connection));
    }
    ...
    void unlock(String name) {
        String finalName = name;
        executeLockFree(connection -> doUnlock(finalName, connection));
    }
