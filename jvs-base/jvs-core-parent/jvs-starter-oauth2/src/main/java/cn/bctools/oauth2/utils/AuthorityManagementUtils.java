...
public class AuthorityManagementUtils {
     ...
     * 条件查询
     */
    public static List<UserDto> userSearchSelectedDto(UserSelectedDto dto) {
        SearchUserDto searchUserDto = new SearchUserDto();
        if (!dto.getDepts().isEmpty()) {
            searchUserDto.setDeptIds(dto.getDepts().stream().map(SysDeptDto::getId).collect(Collectors.toList()));
        }
        if (!dto.getGroups().isEmpty()) {
            searchUserDto.setGroupIds(dto.getGroups().stream().map(UserGroupDto::getId).collect(Collectors.toList()));
        }
        if (!dto.getJobs().isEmpty()) {
            searchUserDto.setJobIds(dto.getJobs().stream().map(SysJobDto::getId).collect(Collectors.toList()));
        }
        if (!dto.getRoles().isEmpty()) {
            searchUserDto.setRoleIds(dto.getRoles().stream().map(SysRoleDto::getId).collect(Collectors.toList()));
        }
        if (!dto.getUsers().isEmpty()) {
            searchUserDto.setUserIds(dto.getUsers().stream().map(UserDto::getId).collect(Collectors.toList()));
        }
        return userSearch(searchUserDto);
    }
...
//    }
     ...
     * @return 部门树
     */
    public static SysDeptDto getParentBranchOffice(String deptId) {
        if (ObjectNull.isNull(deptId)) {
            return null;
        }
        R<SysDeptDto> result = DEPT_SERVICE.getParentBranchOffice(deptId);
        return validResult(result, null);
    }
