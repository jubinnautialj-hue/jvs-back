...
public class TreeUtils {
    ...
    public static List<Tree<Object>> tree(List<TreePo> list) {
        return tree(list, DICT_ID_ROOT);
    }
     ...
     * 将数组map对象 转换为 树形,
     *
     * @param list      数组listMap
     * @param rootId    顶级id值    例: -1
     * @param rootKey   顶级Key 例: id
     * @param parentKey 顶级Key 例: id
     * @param nameKey   显示名称 例: name
     * @param sortKey   排序字段 例: sort
     * @return
     */
    public static List<Tree<Object>> tree(List<Map<String, Object>> list, String rootId, String parentKey, String rootKey, String nameKey, String sortKey) {
        return TreeUtil.build(list, rootId, new TreeNodeConfig(), (treeNode, tree) -> {
            ...
            tree.put("value", treeNode.getOrDefault(rootKey, "id").toString());
        });
    }
    ...
    public static <T, K> T list2Tree(Collection<T> nodeList,
        ...
        List<T> rootList = list2Tree(nodeList, Collections.singletonList(rootKey), getKey, getParentKey, setChildList, null);
    ...
    public static <T, K> T list2Tree(Collection<T> nodeList,
        ...
        List<T> rootList = list2Tree(nodeList, Collections.singletonList(rootKey), getKey, getParentKey, setChildList, comparator);
        if (ObjectUtils.isEmpty(rootList)) {
            return null;
        }
        return rootList.get(0);
    ...
    public static <T, K> List<T> list2Tree(Collection<T> nodeList,
                                           ...
                                           @NotNull Function<T, K> getParentKey,
                                           @NotNull BiConsumer<T, List<T>> setChildList) {
        return list2Tree(nodeList, rootKeyList, getKey, getParentKey, setChildList, null);
    }
    ...
    public static <T, K> List<T> list2Tree(Collection<T> nodeList,
        ...
        return list2Tree(nodeList, rootKeyList, getKey, getParentKey, setChildList, next instanceof Comparable, comparator);
    }
    ...
    private static <T, K> List<T> list2Tree(Collection<T> nodeList,
        ...
        Map<K, List<T>> map = stream.collect(Collectors.groupingBy(getParentKey, Collectors.collectingAndThen(
                Collectors.toList(),
                list -> {
                    if (ObjectNull.isNotNull(comparator)) {
