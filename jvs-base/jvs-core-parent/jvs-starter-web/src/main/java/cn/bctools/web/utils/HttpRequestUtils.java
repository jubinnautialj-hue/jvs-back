...
public class HttpRequestUtils {
    ...
    public HttpComponentsClientHttpRequestFactory generateHttpsRequestFactory() {
        try {
            ...
            throw new BusinessException("创建HttpsRestTemplate失败", e);
        }
    }
    ...
    public static <T> T getJsonR(String url, Class<T> tClass, boolean asynchronous) {
        return parseBean(getJson(url, R.class, asynchronous, new HttpHeaders()), tClass);
    }
    ...
    public static <T> T postJsonR(String url, Map<String, Object> params, Class<T> tClass, boolean asynchronous) {
        return parseBean(postJson(url, params, R.class, asynchronous, new HttpHeaders()), tClass);
    }
    ...
    public static <T> T getJson(String originalUrl, Class<T> tClass, boolean asynchronous, HttpHeaders httpHeaders) {
        return getJson(originalUrl, tClass, asynchronous, httpHeaders, false);
    }
     ...
     * @param asynchronous  是否异步发送
     * @param httpHeaders   请求头
     * @param systemHeaders 是否添加系统请求头
     * @return 响应数据, 异步时返回null
     */
    public static <T> T getJson(String originalUrl, Class<T> tClass, boolean asynchronous, HttpHeaders httpHeaders, Boolean systemHeaders) {
        final String url = handleUrl(originalUrl);
        // 构建请求体
        HttpHeaders headers = systemHeaders ? enhanceHttpHeader() : new HttpHeaders();
        httpHeaders.keySet().forEach(e -> headers.add(e, httpHeaders.getFirst(e)));
    ...
    public static <T> T postJson(String originalUrl, Map<String, Object> params, Class<T> tClass, boolean asynchronous, HttpHeaders httpHeaders) {
        return postJson(originalUrl, params, tClass, asynchronous, httpHeaders, false);
    }
     ...
     * @param asynchronous  是否异步发送
     * @param httpHeaders   请求头
     * @param systemHeaders 是否包含系统请求头
     * @return 响应数据, 异步时返回null
     */
    public static <T> T postJson(String originalUrl, Map<String, Object> params, Class<T> tClass, boolean asynchronous, HttpHeaders httpHeaders, Boolean systemHeaders) {
        final String url = handleUrl(originalUrl);
        // 构建请求体
        HttpHeaders headers = systemHeaders ? enhanceHttpHeader() : new HttpHeaders();
        httpHeaders.keySet().forEach(e -> headers.add(e, httpHeaders.getFirst(e)));
    ...
    public static <T> T execute(String originalUrl, Map<String, Object> params, Class<T> tClass, boolean asynchronous, HttpMethod method, HttpHeaders httpHeaders, Boolean systemHeaders,
        ...
        if (asynchronous) {
            ThreadPoolTaskExecutor executor = SpringContextUtil.getBean(ThreadPoolTaskExecutor.class);
            executor.execute(() -> REST_TEMPLATE.exchange(url, method, requestEntity, tClass));
            return null;
        }
        try {
            ResponseEntity<T> response = REST_TEMPLATE.exchange(url, method, requestEntity, tClass);
            // 获取请求返回值
            return response.getBody();
        } catch (Exception e) {
            if (e instanceof HttpClientErrorException.Unauthorized) {
                throw function.apply((HttpClientErrorException.Unauthorized) e);
            } else {
                throw new BusinessException("http请求发送异常", e.getMessage());
            }
        }
     ...
     * @return 响应数据, 异步时返回null
     */
    public static <T> T postJson(String originalUrl, MultiValueMap<String, Object> params, Class<T> tClass, boolean asynchronous, HttpHeaders httpHeaders) {
        final String url = handleUrl(originalUrl);
        // 构建请求体
        HttpHeaders headers = enhanceFormHttpHeader(httpHeaders);
        final HttpEntity<MultiValueMap<String, Object>> requestEntity = new HttpEntity<>(params, headers);
        // 执行HTTP请求
        if (asynchronous) {
            ThreadPoolTaskExecutor executor = SpringContextUtil.getBean(ThreadPoolTaskExecutor.class);
            executor.execute(() -> REST_TEMPLATE.exchange(url, HttpMethod.POST, requestEntity, tClass));
    ...
    private static <T> T parseBean(R<?> r, Class<T> tClass) {
        ...
        return BeanCopyUtil.copy(r.getData(), tClass);
    }
     ...
     */
    @SneakyThrows
    private static HttpHeaders enhanceFormHttpHeader(HttpHeaders headers) {
        HttpHeaders httpHeaders = ObjectNull.isNull(headers) ? new HttpHeaders() : headers;
        // 设置请求为json格式
