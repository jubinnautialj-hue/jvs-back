...
public class MongoHelper {
     ...
     * 缓存Class相关filed
     */
    final Map<Class, Field[]> classFieldCacheMap = new HashMap<>(8);
    /**
     * 缓存Field相关注解信息
     */
    final Map<Field, FieldAnnotationCache> fieldAnnotationCacheMap = new HashMap<>(8);
    @Autowired
    ...
    public <T> Update createUpdate(T updateObject) {
        ...
        ClassUtils.consumerNotNullField(updateObject, (field -> {
            try {
                ...
                String id = "id";
                String id1 = "_id";
                if (!(id.equals(name) || id1.equals(name))) {
                    update.set(name, field.get(updateObject));
                }
