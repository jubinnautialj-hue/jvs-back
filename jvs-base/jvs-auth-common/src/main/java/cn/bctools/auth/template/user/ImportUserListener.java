...
public class ImportUserListener implements ReadListener<UserExcelTemplate> {
    ...
    public ImportUserListener(UserDto userDto, String defaultPassword, List<SysDeptDto> deptTree, List<SysJobDto> sysJobDtos) {
     ...
     * Map<岗位名称, 岗位id>>
     */
    private Map<String, String> jobCache = new HashMap<>(1);
    /**
     * 需要自动创建的岗位
     */
    ...
    public void invoke(UserExcelTemplate data, AnalysisContext context) {
        ...
        //如果手机号存在，并帐号不存在，则把手机号做为帐号
        if (ObjectNull.isNotNull(data.getPhone()) && ObjectNull.isNull(data.getAccountName())) {
            data.setAccountName(data.getPhone());
        }
        // 校验数据
        checkData(data);
        // 加入缓存
    ...
    private void checkData(UserExcelTemplate data) {
        ...
        if (!UserCurrentUtils.getCurrentUser().getTenant().getTopTenant()) {
            if (ObjectNull.isNull(data.getPhone())) {
                errorMsg = "必须填写手机号";
            }
        }
    ...
    private void saveUser() {
        ...
        if (CollectionUtils.isNotEmpty(phones)) {
            long count = userService.count(Wrappers.<User>lambdaQuery().in(User::getPhone, phones));
            if (count > 0) {
                throw new BusinessException("手机号已存在");
            }
        }
        ...
        // 校验账号是否已存在
        Map<String, Long> accountGroup = users.stream().filter(u -> StringUtils.isNotBlank(u.getAccountName())).collect(Collectors.groupingBy(User::getAccountName, Collectors.counting()));
        long accountGroupNum = accountGroup.values().stream().filter(v -> v > 1).count();
        if (accountGroupNum > 0) {
