...
public class WxEnterpriseDept {
    ...
    public List<Dept> getDeptAll(String accessToken) {
        ...
        List<Dept> deptList = (List<Dept>) deptSimples.parallelStream().map(d -> {
            ...
            return new Dept().setId(dept.getString(DEPT_ID)).setName(dept.getString(DEPT_NAME)).setSort(dept.getIntValue(DEPT_ORDER)).setParentId(parentId).setSource(AuthSourceEnum.WECHAT_ENTERPRISE_WEB.name());
        }).collect(Collectors.toList());
        deptList.removeIf(d -> PARENT_ID.equals(d.getParentId()));
    ...
    private JSONArray getDeptSimples(String accessToken) {
        String deptSimpleUrl = UrlBuilder.fromBaseUrl(DEPT_ID_LIST_URL).queryParam(BaseWxEnterprise.ACCESS_TOKEN, accessToken).build();
        JSONObject jsonObject = HttpRequestUtils.getJson(deptSimpleUrl, JSONObject.class, Boolean.FALSE, new HttpHeaders());
