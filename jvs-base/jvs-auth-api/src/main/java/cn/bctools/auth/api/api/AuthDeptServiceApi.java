...
public interface AuthDeptServiceApi {
     ...
     * 以树的形式返回数据
     *
     * @return 部门信息(树结构) all tree
     */
    @GetMapping(PREFIX + "/query/all/tree")
    R<List<SysDeptDto>> getAllTree();

    /**
     * 获取某一个部分 id下所有的树形结构，包含当前部门
     *
     * @param deptId 部门 id
     * @return dept tree
     */
    @GetMapping(PREFIX + "/query/dept/tree")
    R<List<SysDeptDto>> getDeptTree(@ApiParam("部门id") @PathVariable("deptId") String deptId);

    /**
     * 查询直接下级部门信息
     *
     * @param deptId 部门id
     * @return 下级部门信息集合 child list
     */
     ...
     *
     * @param deptId 部门id
     * @return 返回部门信息 parent branch office
     */
