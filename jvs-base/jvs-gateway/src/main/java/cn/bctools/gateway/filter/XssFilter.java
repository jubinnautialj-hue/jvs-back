...
public class XssFilter implements GlobalFilter, Ordered {
    ...
    @Autowired
    GatewayIgnoreXssMapper ignoreXssMapper;
            ...
            return DataBufferUtils.join(request.getBody().defaultIfEmpty(factory.wrap(new EmptyByteBuf(factory.getByteBufAllocator()))))
                    .flatMap(dataBuffer -> {
                        ...
                        if (bodyString2.length() != stlen) {
                            throw new BusinessException("请求被xxs拦截请联系管理员");
                        }
    ...
    private boolean white(String url) {
        return CacheUtils.cache.get(CacheUtils.Type.xss, () -> ignoreXssMapper.selectList(Wrappers.query()))
                .stream()
                .map(e -> ((GatewayIgnoreXss) e).getPath())
                .anyMatch(e -> PATH_MATCHER.matchStart(String.valueOf(e), url));
    }
