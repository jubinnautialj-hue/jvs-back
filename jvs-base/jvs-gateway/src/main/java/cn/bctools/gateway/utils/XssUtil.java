...
public enum XssUtil {
    ...
    private String cleanArr(String val) {
        List<Object> list = JSONArray.parseArray(val);
        List<Object> result = new ArrayList<>(list.size());
        for (Object str : list) {
            if (JSON.isValidObject(str.toString())) {
                //对象
                str = JSONObject.parse(this.cleanXss(str));
            } else if (JSON.isValidArray(str.toString())) {
                //数组
                str = JSONArray.parseArray(this.cleanXss(str));
            } else {
                //字符
                str = (this.cleanXss(str));
            }
