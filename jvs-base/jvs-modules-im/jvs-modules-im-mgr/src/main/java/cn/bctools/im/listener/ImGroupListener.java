...
public class ImGroupListener extends AbstractImGroupListener {
    ...
    public void doAfterBind(ImChannelContext imChannelContext, Group group) throws ImException {
        logger.info("群组:{},绑定成功!", group.getGroupId());
    ...
    public void doAfterUnbind(ImChannelContext imChannelContext, Group group) throws ImException {
        String mark = ChannelMarkUtil.buildMark(imChannelContext.getTenantId(), group.getGroupId());
        ...
        JimServerAPI.sendToGroup(mark, imPacket);
    }
    ...
    public void joinGroupNotify(Group group, ImChannelContext imChannelContext) throws ImException {
        ...
        User notifyUser = User.newBuilder().userId(clientUser.getUserId()).nick(clientUser.getNick()).avatar(clientUser.getAvatar()).status(UserStatusType.ONLINE.getStatus()).build();
        //发进房间通知  COMMAND_JOIN_GROUP_NOTIFY_RESP
        JoinGroupNotifyRespBody joinGroupNotifyRespBody = JoinGroupNotifyRespBody.success();
        joinGroupNotifyRespBody.setGroup(group.getGroupId()).setUser(notifyUser);
        //给分组发送信息
        RespBody respBody = new RespBody(Command.COMMAND_JOIN_GROUP_NOTIFY_RESP, joinGroupNotifyRespBody);
        ImPacket imPacket = new ImPacket(Command.COMMAND_JOIN_GROUP_NOTIFY_RESP, respBody.toByte());
        JimServerAPI.sendToGroup(mark, imPacket);
    }
