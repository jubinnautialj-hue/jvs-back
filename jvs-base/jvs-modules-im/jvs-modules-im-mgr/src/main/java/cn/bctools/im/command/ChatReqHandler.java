...
public class ChatReqHandler extends AbstractCmdHandler {
	...
	public ImPacket handler(ImPacket packet, ImChannelContext channelContext) throws ImException {
		...
		chatBody.setTenantId(ChannelTenantUtil.build(channelContext.getTenantId(), chatBody.getTenantId()));
		if (StringUtils.isBlank(chatBody.getTenantId())) {
			return ProtocolManager.Packet.dataInCorrect(Command.COMMAND_CHAT_RESP, channelContext);
		}
		packet.setBody(chatBody.toByte());
		//聊天数据格式不正确
		if(Boolean.FALSE.equals(checkBody(chatBody))){
		...
		RespBody respBody = new RespBody(Command.COMMAND_CHAT_RESP, chatBody);
		ImPacket chatPacket = new ImPacket(Command.COMMAND_CHAT_RESP, respBody.toByte());
		// 设置同步序列号
		...
		if(ChatType.CHAT_TYPE_PRIVATE.getNumber() == chatBody.getChatType()){
			...
			if(ChatKit.isOnline(chatBody.getTenantId(), toId, isStore)){
				JimServerAPI.sendToUser(ChannelMarkUtil.buildMark(chatBody.getTenantId(), toId), chatPacket);
				// 发送成功响应包
		...
		// 群聊
			...
			String groupId = ChannelMarkUtil.buildMark(chatBody.getTenantId(), chatBody.getGroupId());
	 ...
	 */
	public Boolean checkBody(ChatBody chatBody) {
		if(chatBody == null || ChatType.forNumber(chatBody.getChatType()) == null){
			return Boolean.FALSE;
		}
		// 群聊，目标群组id不能为空
		if (ChatType.CHAT_TYPE_PUBLIC.getNumber() == chatBody.getChatType() && StringUtils.isEmpty(chatBody.getGroupId())) {
			return Boolean.FALSE;
		}
		// 私聊，目标用户id不能为空
		if (ChatType.CHAT_TYPE_PRIVATE.getNumber() == chatBody.getChatType() && StringUtils.isEmpty(chatBody.getTo())) {
			return Boolean.FALSE;
		}
		// 聊天内容不能为空
		if (StringUtils.isEmpty(chatBody.getContent())) {
			return Boolean.FALSE;
		}
