...
/**
 * @Author: ZhuXiaoKang
 * @Description: 组内用户id集合处理
 */
@Slf4j
@Service
@AllArgsConstructor
public class ChannelDataGroupUserIdsServiceImpl implements ChannelDataService {

    private final ImChannelDataService imChannelDataService;

    @Override
    public String getType() {
        return ChannelDataBusinessTypeEnum.GROUP_USER_IDS.getValue();
    }
    ...
    public Boolean process(ChannelData channelData) {
        try {
            ...
            if (ChannelDataStoreTypeEnum.REDIS.getValue().equals(channelData.getStoreType())) {
                JedisTemplate.me().listRemove(channelData.getStoreCode(), 0, channelData.getUserId());
            }
            return Boolean.TRUE;
        } catch (Exception e) {
            log.error("组内用户id:{},处理异常: {}", channelData.getUserId(), e.getMessage());
        }
