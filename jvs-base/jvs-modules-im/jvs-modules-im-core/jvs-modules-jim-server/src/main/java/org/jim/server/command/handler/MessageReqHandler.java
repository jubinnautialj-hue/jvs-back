...
public class MessageReqHandler extends AbstractCmdHandler {
	...
	public ImPacket handler(ImPacket packet, ImChannelContext imChannelContext) throws ImException {
		...
		if(!StringUtils.isEmpty(groupId)){
			//离线消息;
			if(0 == type){
				messageData = messageHelper.getGroupOfflineMessage(tenantId,userId,groupId);
			//历史消息;
			}else if(1 == type){
				messageData = messageHelper.getGroupHistoryMessage(tenantId, userId, groupId,beginTime,endTime,offset,count);
			}
		}else if(StringUtils.isEmpty(fromUserId)){
			//获取用户所有离线消息(好友+群组);
			if(0 == type){
				messageData = messageHelper.getFriendsOfflineMessage(tenantId, userId);
			}else{
				return getMessageFailedPacket(imChannelContext);
			}
		}else{
			//获取与指定用户离线消息;
			if(0 == type){
				messageData = messageHelper.getFriendsOfflineMessage(tenantId, userId, fromUserId);
			//获取与指定用户历史消息;
			}else if(1 == type){
				messageData = messageHelper.getFriendHistoryMessage(tenantId, userId, fromUserId,beginTime,endTime,offset,count);
			}
		}
