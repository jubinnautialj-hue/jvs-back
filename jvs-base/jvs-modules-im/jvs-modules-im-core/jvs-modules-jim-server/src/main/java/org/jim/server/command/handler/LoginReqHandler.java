...
public class LoginReqHandler extends AbstractCmdHandler {
     ...
     * @throws ImException
     */
    private <T extends User> T getUserByProcessor(ImChannelContext imChannelContext, LoginCmdProcessor loginProcessor, LoginReqBody loginReqBody, LoginRespBody loginRespBody) throws ImException {
//		if(Objects.isNull(loginProcessor)){
//			T user = User.newBuilder().userId(loginReqBody.getUserId()).status(UserStatusType.ONLINE.getStatus()).build();
//			return user;
//		}
        ...
        if (Objects.isNull(loginRespBody) || loginRespBody.getCode() != ImStatus.C10007.getCode()) {
            log.error("login failed, getLogType:{}, getValue:{}", loginReqBody.getLogType(), loginReqBody.getValue());
    ...
    public void initGroup(ImChannelContext imChannelContext, User user) throws ImException {
        ...
        if (isStore) {
            groupIds = messageHelper.getGroups(imChannelContext.getTenantId(), userId);
        }
        //绑定群组
