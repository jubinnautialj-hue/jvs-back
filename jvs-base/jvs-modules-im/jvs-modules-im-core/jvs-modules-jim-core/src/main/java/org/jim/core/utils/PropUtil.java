...
public class PropUtil {
    ...
    public static void setProp(Prop prop) {
        PropUtil.prop = prop;
    }

    public static ConcurrentHashMap<String, Prop> getMap() {
        return map;
    }
    ...
    public static Prop use(Map<String, String> pro) {
        String defaultKey = "map";
        Prop result = map.get(defaultKey);
        if (result == null) {
            result = new Prop(pro);
            map.put(defaultKey, result);
            if (prop == null) {
                prop = result;
            }
        }
