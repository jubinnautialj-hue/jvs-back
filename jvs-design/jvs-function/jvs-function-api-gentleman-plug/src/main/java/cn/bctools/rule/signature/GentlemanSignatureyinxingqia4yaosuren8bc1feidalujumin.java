...
import cn.bctools.rule.dto.GentlemanSignatureyinxingqia4yaosuren8bc1feidalujuminDto;
...
/**
 * The type Gentleman signatureyinxingqia 4 yaosuren 8 bc 1 feidalujumin.
 *
 * @author jvs
 */
@Rule(value = "银行卡四要素认证非大陆居民",
        group = RuleGroup.君子签,
        test = true,
        returnType = ClassType.对象,
        testShowEnum = TestShowEnum.JSON,
        order = 2,
        explain = "验证银行卡卡号、姓名、证件号、银行预留手机号是否一致，支持借记卡和贷记卡。请注意：每张银行卡每日调用最多5次；不同银行卡同一身份证号最多10次，超了就会报错，超限。该接口涉及到身份认证费用，如果未购买需要联系商务进行购买。"
)
@AllArgsConstructor
public class GentlemanSignatureyinxingqia4yaosuren8bc1feidalujumin implements BaseCustomFunctionInterface<GentlemanSignatureyinxingqia4yaosuren8bc1feidalujuminDto> {

    @Override
    public Object execute(GentlemanSignatureyinxingqia4yaosuren8bc1feidalujuminDto dto, Map<String, Object> params) {
        return JunZiQianUtils.post("/v2/auth/otherIdNo/bankFourVerify", dto);
    }

    @Override
    public List<RuleElementVo> structureType(GentlemanSignatureyinxingqia4yaosuren8bc1feidalujuminDto o) {
        ...
        RuleElementVo code = new RuleElementVo().setName("code").setInfo("0成功").setJvsParamType(JvsParamType.number).setJvsParamTypeName(JvsParamType.number.getDesc());
        list.add(code);
        return list;
    }
}
