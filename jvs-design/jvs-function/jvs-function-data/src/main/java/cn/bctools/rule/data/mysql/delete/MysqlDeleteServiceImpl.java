package cn.bctools.rule.data.mysql.delete;
...
@AllArgsConstructor
public class MysqlDeleteServiceImpl implements BaseCustomFunctionInterface<MysqlDeleteDto> {

    ModelInterface modelInterface;

    @Override
    public Object execute(MysqlDeleteDto dto, Map<String, Object> params) {
        if (StrUtil.isNotEmpty(dto.getExecSql())) {
            if (!StrUtil.startWithIgnoreCase(dto.getExecSql(), "delete")) {
                throw new BusinessException("当前节点为Mysql删除节点，请检查SQL执行语句关键字");
            }
        }
