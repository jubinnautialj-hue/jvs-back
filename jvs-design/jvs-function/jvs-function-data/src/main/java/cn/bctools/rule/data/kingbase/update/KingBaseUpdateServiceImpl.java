package cn.bctools.rule.data.kingbase.update;
...
@AllArgsConstructor
public class KingBaseUpdateServiceImpl implements BaseCustomFunctionInterface<KingBaseUpdateDto> {

    ModelInterface modelInterface;

    @Override
    public Object execute(KingBaseUpdateDto dto, Map<String, Object> params) {

        if (StrUtil.isNotEmpty(dto.getExecSql())) {
            if (!StrUtil.startWithIgnoreCase(dto.getExecSql(), "update")) {
                throw new BusinessException("KingBase更新节点，请检查SQL执行语句关键字");
            }
        }
