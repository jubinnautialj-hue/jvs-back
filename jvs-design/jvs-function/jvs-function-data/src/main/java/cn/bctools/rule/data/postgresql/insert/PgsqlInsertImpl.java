package cn.bctools.rule.data.postgresql.insert;
...
@Slf4j
@Service
@Rule(value = "postgresql插入",
        group = RuleGroup.数据插件,
        test = true,
        returnType = ClassType.数字,
        testShowEnum = TestShowEnum.JSON,
        order = 45,
        explain = "postgresql插入"
)
@RequiredArgsConstructor
public class PgsqlInsertImpl implements BaseCustomFunctionInterface<PgsqlBaseDto> {
    ...
    public Object execute(PgsqlBaseDto dto, Map<String, Object> params) {
        ...
        if (StrUtil.isNotEmpty(dto.getExecSql())) {
            if (!StrUtil.startWithIgnoreCase(dto.getExecSql(), "insert")) {
                throw new BusinessException("当前节点为postgresql插入，请检查SQL执行语句关键字");
            }
        }
        ...
        try {
            return ClientConfig.init(option, pgsql_templateFunction).insertData(replace);
        } catch (Exception e) {
        }
        return null;
    }
