package cn.bctools.rule.data.sqlserver.update;
...
@AllArgsConstructor
public class SqlServerUpdateServiceImpl extends SqlServerBase implements BaseCustomFunctionInterface<SqlServerUpdateDto> {
    ModelInterface modelInterface;

    @Override
    public Object execute(SqlServerUpdateDto dto, Map<String, Object> params) {
        if (StrUtil.isNotEmpty(dto.getExecSql())) {
            if (!StrUtil.startWithIgnoreCase(dto.getExecSql(), "update")) {
                throw new BusinessException("当前节点为SqlServer更新节点，请检查SQL执行语句关键字");
            }
        }
