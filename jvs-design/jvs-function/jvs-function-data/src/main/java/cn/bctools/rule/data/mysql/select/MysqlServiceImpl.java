package cn.bctools.rule.data.mysql.select;
...
public class MysqlServiceImpl implements BaseCustomFunctionInterface<MysqlFunctionDto> {
    ...
    @Override
    public Object execute(MysqlFunctionDto dto, Map<String, Object> params) {
        if (StrUtil.isNotEmpty(dto.getExecSql())) {
            if (!StrUtil.startWithIgnoreCase(dto.getExecSql(), "select")) {
                throw new BusinessException("当前节点为Mysql查询节点，请检查SQL执行语句关键字");
            }
        }
        ...
        try {
            return ClientConfig.init(option, mysql_templateFunction).queryData(replace, dto.getResultType());
