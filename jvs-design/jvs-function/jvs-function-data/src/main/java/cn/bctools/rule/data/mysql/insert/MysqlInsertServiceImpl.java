package cn.bctools.rule.data.mysql.insert;
...
@AllArgsConstructor
public class MysqlInsertServiceImpl implements BaseCustomFunctionInterface<MysqlInsertDto> {
    ModelInterface modelInterface;


    @Override
    public Object execute(MysqlInsertDto dto, Map<String, Object> params) {
        if (StrUtil.isNotEmpty(dto.getExecSql())) {
            if (!StrUtil.startWithIgnoreCase(dto.getExecSql(), "insert")) {
                throw new BusinessException("当前节点为Mysql新增节点，请检查SQL执行语句关键字");
            }
        }
        ...
        try {
            return ClientConfig.init(option, mysql_templateFunction).insertData(replace);
