package cn.bctools.rule.data.postgresql.delete;
...
@Slf4j
@Service
@Rule(value = "postgresql删除",
        group = RuleGroup.数据插件,
        test = true,
        returnType = ClassType.数字,
        testShowEnum = TestShowEnum.JSON,
        order = 44,
        explain = "postgresql删除"
)
@RequiredArgsConstructor
public class PgsqlDeleteImpl implements BaseCustomFunctionInterface<PgsqlBaseDto> {
    ...
    public Object execute(PgsqlBaseDto dto, Map<String, Object> params) {
        if (StrUtil.isNotEmpty(dto.getExecSql())) {
            if (!StrUtil.startWithIgnoreCase(dto.getExecSql(), "delete")) {
                throw new BusinessException("当前节点为postgresql删除，请检查SQL执行语句关键字");
            }
        }
