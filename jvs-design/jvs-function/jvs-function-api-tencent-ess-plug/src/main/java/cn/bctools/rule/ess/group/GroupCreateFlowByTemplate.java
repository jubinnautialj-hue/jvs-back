...
public class GroupCreateFlowByTemplate {
    ...
    /**
     * Create flow create flow response.
     *
     * @param operatorId          the operator id
     * @param proxyOrganizationId the proxy organization id
     * @param flowName            the flow name
     * @param approvers           the approvers
     * @return the create flow response
    ...
    /**
     * Create document create document response.
     *
     * @param operatorId          the operator id
     * @param proxyOrganizationId the proxy organization id
     ...
     * @return the create document response
     * @throws TencentCloudSDKException the tencent cloud sdk exception
     */
    ...
    public static void main(String[] args) {
        try {
            ...
            CreateFlowResponse createFlowResponse = createFlow(Config.OPERATOR_USER_ID, proxyOrganizationId,flowName, approvers);
            String flowId = createFlowResponse.getFlowId();
            log.info(new Gson().toJson(createFlowResponse));

            // 2、调用CreateDocument接口创建文档
            CreateDocumentResponse createDocumentResponse = createDocument(Config.OPERATOR_USER_ID, proxyOrganizationId, flowId, templateId, fileName );
            log.info(new Gson().toJson(createDocumentResponse));

            TimeUnit.SECONDS.sleep(3);

            // 3、调用StartFlow接口开启流程
            StartFlowResponse startFlowResponse = startFlow(Config.OPERATOR_USER_ID, proxyOrganizationId,flowId);
            log.info(new Gson().toJson(startFlowResponse));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
