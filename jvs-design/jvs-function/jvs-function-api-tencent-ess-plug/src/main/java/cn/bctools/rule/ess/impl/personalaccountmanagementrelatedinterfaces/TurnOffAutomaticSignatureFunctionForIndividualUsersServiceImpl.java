...
//        iconUrl = "rule-dysmsduanxinfuwu",
        explain = "通过此接口可以关闭个人用户自动签功能。 无需对应的用户刷脸等方式同意即可关闭。\n" +
                "\n" +
                "注意:\n" +
                "\n" +
...
@AllArgsConstructor
public class TurnOffAutomaticSignatureFunctionForIndividualUsersServiceImpl implements BaseCustomFunctionInterface<TurnOffAutomaticSignatureFunctionForIndividualUsersDto> {

    @Override
    public Object execute(TurnOffAutomaticSignatureFunctionForIndividualUsersDto dto, Map<String, Object> params) {
        EssClient client = TenantUtil.getClient(dto.getOptions());
        try {
            // 实例化一个请求对象,每个接口都会对应一个request对象
            DisableUserAutoSignRequest req = BeanCopyUtil.copy(dto, DisableUserAutoSignRequest.class);
            req.setOperator(BeanCopyUtil.copy(dto.getOperator(), UserInfo.class));
            // 返回的resp是一个DisableUserAutoSignResponse的实例，与请求对象对应
            DisableUserAutoSignResponse resp = client.DisableUserAutoSign(req);
