...
//        iconUrl = "rule-dysmsduanxinfuwu",
        explain = "此接口（CreateFlowGroupByFiles）可用于通过多个文件创建合同组签署流程。\n" +
                "\n" +
                "适用场景：该接口适用于需要一次性完成多份合同签署的情况，多份合同一般具有关联性，用户以目录的形式查看合同。"
)
@AllArgsConstructor
public class CreateContractGroupSigningProcessThroughMultipleFilesServiceImpl implements BaseCustomFunctionInterface<CreateContractGroupSigningProcessThroughMultipleFilesDto> {

    @Override
    public Object execute(CreateContractGroupSigningProcessThroughMultipleFilesDto dto, Map<String, Object> params) {
        EssClient client = TenantUtil.getClient(dto.getOptions());
        // 实例化一个请求对象,每个接口都会对应一个request对象
        try {
            // 实例化一个请求对象,每个接口都会对应一个request对象
            CreateFlowGroupByFilesRequest req = new CreateFlowGroupByFilesRequest();
            ...
            // 返回的resp是一个CreateFlowGroupByFilesResponse的实例，与请求对象对应
            CreateFlowGroupByFilesResponse resp = client.CreateFlowGroupByFiles(req);
