...
public class InterfaceForQueryingPersonalCertificatesDto {
    ...
    @ParameterValue(info = "(UserInfo)个人用户的三要素信息：\n" +
            "姓名\n" +
            "证件号\n" +
            "证件类型", necessity = false, type = InputType.map)
    @SerializedName("UserInfo")
    @Expose
    public Map userInfo;

    @ParameterValue(info = "(SceneKey)证书使用场景，可以选择的场景值如下:\n" +
