...
public class CreateContractGroupSigningProcessThroughMultipleTemplatesDto {
    ...
    public Map operator;
    @ParameterValue(info = "(FlowGroupName)合同（流程）组名称（可自定义此名称），长度不能超过200，只能由中文、字母、数字和下划线组成。", necessity = false, type = InputType.input)
    /**
    ...
    @ParameterValue(info = "(FlowGroupInfos)合同（流程）组的子合同信息，支持2-50个子合同。", type = InputType.listMap)
    ...
    @ParameterValue(info = "(FlowGroupOptions)合同（流程）组的配置项信息。是否通知本企业签署方。是否通知其他签署方.", necessity = false, type = InputType.map)
    /**
     * 合同（流程）组的配置项信息。
     其中包括：
     <ul>
