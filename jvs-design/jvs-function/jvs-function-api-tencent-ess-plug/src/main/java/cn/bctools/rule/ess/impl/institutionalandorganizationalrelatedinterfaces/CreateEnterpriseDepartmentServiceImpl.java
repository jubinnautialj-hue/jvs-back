...
@AllArgsConstructor
public class CreateEnterpriseDepartmentServiceImpl implements BaseCustomFunctionInterface<CreateEnterpriseDepartmentDto> {

    @Override
    public Object execute(CreateEnterpriseDepartmentDto dto, Map<String, Object> params) {
        EssClient client = TenantUtil.getClient(dto.getOptions());
        try {
            // 实例化一个请求对象,每个接口都会对应一个request对象
            CreateIntegrationDepartmentRequest req = BeanCopyUtil.copy(dto, CreateIntegrationDepartmentRequest.class);
            req.setOperator(BeanCopyUtil.copy(dto.getOperator(), UserInfo.class));

            // 返回的resp是一个CreateIntegrationDepartmentResponse的实例，与请求对象对应
            CreateIntegrationDepartmentResponse resp = client.CreateIntegrationDepartment(req);
