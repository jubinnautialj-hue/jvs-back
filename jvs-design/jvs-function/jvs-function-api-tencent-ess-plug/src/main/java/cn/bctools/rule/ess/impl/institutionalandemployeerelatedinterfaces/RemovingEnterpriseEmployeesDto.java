...
public class RemovingEnterpriseEmployeesDto {
    ...
    @ParameterValue(info = "(Employees)待移除员工的信息。应符合以下规则：\n" +
            "UserId和OpenId不可同时为空。\n" +
            "若需要进行离职交接，交接人信息ReceiveUserId和ReceiveOpenId不可同时为空。否则视为不进行离职交接。", type = InputType.listMap)
