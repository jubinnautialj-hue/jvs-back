 ...
 * @author gl
 */
@Rule(value = "员工Userid与客户系统Openid绑定",
...
//        iconUrl = "rule-dysmsduanxinfuwu",
        explain = "此接口（BindEmployeeUserIdWithClientOpenId）用于将电子签系统员工UserId与客户系统员工OpenId进行绑定。"
)
@AllArgsConstructor
public class BindingEmployeeUseridToCustomerSystemOpenidServiceImpl implements BaseCustomFunctionInterface<BindingEmployeeUseridToCustomerSystemOpenidDto> {

    @Override
    public Object execute(BindingEmployeeUseridToCustomerSystemOpenidDto dto, Map<String, Object> params) {
        EssClient client = TenantUtil.getClient(dto.getOptions());
        try {
            // 实例化一个请求对象,每个接口都会对应一个request对象
            BindEmployeeUserIdWithClientOpenIdRequest req = BeanCopyUtil.copy(dto, BindEmployeeUserIdWithClientOpenIdRequest.class);
            req.setOperator(BeanCopyUtil.copy(dto.getOperator(), UserInfo.class));

            // 返回的resp是一个BindEmployeeUserIdWithClientOpenIdResponse的实例，与请求对象对应
            BindEmployeeUserIdWithClientOpenIdResponse resp = client.BindEmployeeUserIdWithClientOpenId(req);
