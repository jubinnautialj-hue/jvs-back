...
//        iconUrl = "rule-dysmsduanxinfuwu",
        explain = "本接口（CreatePreparedPersonalEsign）用于创建导入个人印章（处方单场景专用，使用此接口请与客户经理确认）。"
)
@AllArgsConstructor
public class CreatePersonalSealForImportingPrescriptionFormsServiceImpl implements BaseCustomFunctionInterface<CreatePersonalSealForImportingPrescriptionFormsDto> {

    @Override
    public Object execute(CreatePersonalSealForImportingPrescriptionFormsDto dto, Map<String, Object> params) {
        ...
        try {
            ...
            // 返回的resp是一个CreatePreparedPersonalEsignResponse的实例，与请求对象对应
            CreatePreparedPersonalEsignResponse resp = client.CreatePreparedPersonalEsign(req);
