...
//        iconUrl = "rule-dysmsduanxinfuwu",
        explain = "本接口（CreateSealPolicy）用于对企业员工进行印章授权"
)
@AllArgsConstructor
public class CreateAuthorizationForEnterpriseEmployeeSealsServiceImpl implements BaseCustomFunctionInterface<CreateAuthorizationForEnterpriseEmployeeSealsDto> {

    @Override
    public Object execute(CreateAuthorizationForEnterpriseEmployeeSealsDto dto, Map<String, Object> params) {
        EssClient client = TenantUtil.getClient(dto.getOptions());
        try {
            // 实例化一个请求对象,每个接口都会对应一个request对象
            CreateSealPolicyRequest req = BeanCopyUtil.copy(dto, CreateSealPolicyRequest.class);
            req.setOperator(BeanCopyUtil.copy(dto.getOperator(), UserInfo.class));
            // 返回的resp是一个CreateSealPolicyResponse的实例，与请求对象对应
            CreateSealPolicyResponse resp = client.CreateSealPolicy(req);
