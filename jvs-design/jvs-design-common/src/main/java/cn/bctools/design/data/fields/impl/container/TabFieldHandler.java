    ...
    public void isNext(TabItemHtml tabItemHtml) {
        if (!tabItemHtml.getColumn().isEmpty()) {
            ...
            for (String s : column.keySet()) {
                ...
                //判断同级是否有key重复
                Map<String, Integer> listMap = list.stream().map(e -> e.getProp()).collect(Collectors.toMap(qe -> qe, qe -> 1, Integer::sum));
                for (String key : listMap.keySet()) {
                    if (listMap.get(key) > 1) {
                        throw new BusinessException("下有重复的字段命名请修改", tabItemHtml.getLabel(), key);
                    }
                }
    ...
    public void next(List<ElementVo> list, TabItemHtml html, Map<String, IDataFieldHandler> handlerMap, ElementVo e) {
        ...
        for (FormValueHtml dicDatum : html.getDicData()) {
            ...
            if (html.getDetachData()) {
                ...
                ElementVo elementVo = e;
                if (ObjectNull.isNotNull(prop)) {
                    elementVo = new ElementVo()
                            .setId(prop)
                            .setPath(e.getPath() + StrUtil.DOT + dicDatum.getName())
                            .setName(dicDatum.getLabel() + StrUtil.DOT + prop).setShortName(prop).setInfo(dicDatum.getLabel() + StrUtil.DOT + prop).setJvsParamType(JvsParamType.object);
                    list.add(elementVo);
                }
                for (FieldBasicsHtml baseItemHtml : publicHtmls) {
                    ...
                    if (ObjectNull.isNotNull(prop)) {
                        //如果不为空，公式数据为属性名加值
                        paramType = new ElementVo()
                                .setId(prop + StrUtil.DOT + baseItemHtml.getProp())
                                .setPath(e.getPath() + StrUtil.DOT + dicDatum.getName() + StrUtil.DOT + baseItemHtml.getProp())
                                .setType("请求入参")
                                .setName(prop + StrUtil.DOT + baseItemHtml.getLabel())
                                .setFieldType(baseItemHtml.getType().name())
                                .setShortName(dicDatum.getProp() + StrUtil.DOT + baseItemHtml.getLabel())
                                .setJvsParamType(JvsParamType.getByClass(baseItemHtml.getType().getAClass()));
                        list.add(paramType);
                    } else {
                        paramType = new ElementVo()
                                .setId(baseItemHtml.getProp())
                                .setPath(e.getPath() + StrUtil.DOT + dicDatum.getName() + StrUtil.DOT + baseItemHtml.getProp())
                                .setType("请求入参").setName(baseItemHtml.getLabel()).setFieldType(baseItemHtml.getType().name()).setShortName(baseItemHtml.getLabel()).setJvsParamType(JvsParamType.getByClass(baseItemHtml.getType().getAClass()));
                        list.add(paramType);
                    }
                ...
                ElementVo elementVo = new ElementVo()
                        .setId(e.getId() + StrUtil.DOT + dicDatum.getName())
                        .setPath(e.getPath() + StrUtil.DOT + dicDatum.getName())
                        .setFieldType(html.getType().name())
                        .setType("请求入参")
                        .setName(e.getName() + StrUtil.DOT + dicDatum.getLabel())
                        .setShortName(e.getShortName() + StrUtil.DOT + dicDatum.getLabel())
                ...
                for (FieldPublicHtml baseItemHtml : publicHtmls) {
                    ...
                    ElementVo paramType = new ElementVo()
                            ...
                            .setPath(e.getPath() + StrUtil.DOT + baseItemHtml.getName())
        ...
        if (fieldDto.getDetachData()) {
            ...
            for (FormValueHtml dicDatum : fieldDto.getDicData()) {
                ...
                if (ObjectNull.isNotNull(field)) {
                    ...
                    //将返回的对象根据数据类型进行解析
                    for (String fieldKey : fieldPublicHtmlMap.keySet()) {
            ...
            for (String key : column.keySet()) {
                if (ObjectNull.isNull(column.get(key))) {
                    continue;
                }
                Map<String, FieldPublicHtml> fieldPublicHtmls = column.get(key).stream().collect(Collectors.toMap(FieldPublicHtml::getProp, Function.identity()));
