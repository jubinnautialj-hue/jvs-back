...
/**
 * word文件合并 但是目前只支持 docx
 */
public class WordMergeDocUtils {

    /**
     * 参数1:需要合并的word的文件对象list
     * 参数2:合并之后word存储的全路径file对象
     */
    public static void mergeDoc(List<InputStream> fileList, File newFile) throws Exception {
        OutputStream dest = new FileOutputStream(newFile);
        ArrayList<XWPFDocument> documentList = new ArrayList<XWPFDocument>();
        XWPFDocument doc = null;
        for (int i = 0; i < fileList.size(); i++) {
            OPCPackage open = OPCPackage.open(fileList.get(i));
