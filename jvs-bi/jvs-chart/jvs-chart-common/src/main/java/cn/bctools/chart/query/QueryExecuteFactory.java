package cn.bctools.chart.query;

import cn.bctools.chart.chart.bo.ChartFilterJsonBo;
import cn.bctools.chart.chart.bo.ChartSettingBo;
import cn.bctools.chart.chart.bo.QueryParameterBo;
...
public class QueryExecuteFactory {
    ...
    private String getDataValue(QueryParameterBo queryParameterBo) {
        if (queryParameterBo.getType().equals(QueryParameterTypeEnums.select)) {
            ...
            if (isDay) {
                //对日期格式化
                return JSONObject.toJSONString(Arrays.asList(DateUtil.format(DateUtil.parse(startTime), queryParameterBo.getFormat()), DateUtil.format(DateUtil.parse(endTime), queryParameterBo.getFormat())));
            }
